...
\begin{itemize}
	\item Searching unstructured databases (Sec.~\ref{sec:quantum_search}):\index{Grover's algorithm}
		...
		\end{itemize}
	\item Satisfiability \& optimisation problems\footnote{A satisfiability problem is one where we search a function's input space for a solution(s) satisfying a given output constraint. The hardest such problems, like the archetypal 3-\textsc{SAT}\index{3-SAT problem} problem, are \textbf{NP}-complete.}\index{NP \& NP-complete}\index{Optimisation problems}\index{Satisfiability problems} (Sec.~\ref{sec:optimising_the_world}):
		\begin{itemize}
...
\begin{figure}[!htb]
	\includegraphics[width=0.5\textwidth]{complexity_classes}\index{Complexity classes} \index{P}\index{BPP} \index{NP} \index{BQP} \index{\#P} \index{EXP}

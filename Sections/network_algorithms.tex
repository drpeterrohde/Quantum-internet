...
\fi
\caption{Example of Dijkstra's shortest-path algorithm for finding the shortest route from $A$ to $B$. Initially (\mbox{$t=0$}) all nodes are designated a tentative distance of $\infty$, except the starting node $A$, which is designated distance $0$. At each step we choose the lowest cost path to a previously unexplored node (edge shown in red). This updates the tentative distance of the newly-visited neighbour, which is now marked as visited (red vertex). This is iterated until all nodes have been visited. Each node is visited exactly once.}\label{fig:dijkstra_eg}\index{Shortest-path algorithm}	
\end{figure}

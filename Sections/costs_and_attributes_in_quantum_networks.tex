...
% \captionspacefig \caption{A sequence of events \mbox{$X\to Y\to Z$}.
 The data processing inequality states that the mutual information from beginning to end is upper-bounded by the mutual information between neighbouring stages, as per Eq.~(\ref{eq:data_proc_ineq}).%}%\label{fig:data_proc_ineq}	
...
\end{tabular}
\caption{\label{tab:capacities} Measure of classical and quantum channel capacities.}
...
One of the channels is a 50$\%$ erasure channel (which we know to have zero quantum capacity from the non-cloning theroem), and the other is known as an entanglement-binding channel (
see Ref~\cite{horodecki2001separability} for details of such a channel).

...
From this, we can then define the \comment{accessible information}\index{Accessible information}, $I_c(\hat\rho_{A,B})$ -- the maximum of the mutual information, optimised over \textit{all} possible measurement settings. Suppose $\hat\rho_{A,B}$ is shared between Alice and Bob, from which they would like to extract maximal correlations (i.e joint information). They measure using local measurement bases $\{\Lambda_A^x\}$ and $\{\Lambda_B^x\}$, yielding random variables $A$ and $B$ \comment{Changed from X,Y to A,B}. Their \comment{accessible information} is defined as,
\begin{align}
I_c(\hat\rho_{A,B}) = \max_{\{\Lambda_A^x\},\{\Lambda_B^x\}} I(A;B).
\end{align}
\comment{Previously this was `classical mutual information'. Is that right? Isn't this actually the `accessible information'?}

The mutual information is defined as being between a particular known pair of states. Of course, in a quantum channel we have the flexibility to manipulate the input state and the measurement at the output. Therefore we can optimise the mutual information of a particular channel, which leads to the following definiton:

The \comment{\textit{classical channel capacity}}\index{Classical channel capacity} of quantum channel $\mathcal{E}$ is defined as,
\begin{align}
I(\mathcal{E}) = \max_{p(x)} I(X;Y),
\end{align}
\comment{What's p(x)?}
with the intuitive interpretation as the maximum mutual information between input and output states that can be achieved across the channel. This effectively places a physical upper bound on the achievable bitrate or bandwidth of the channel. For a noisy channel, if the input contains product states (i.e. entanglement between input states is not allowed), the classical channel capacity can be expressed as,
\begin{align}
...
And the \textit{classical channel capacity} of $\mathcal{E}$ is defined as
...
which can be interpreted as the maximum bitrate of the channel, per use of the channel, in the limit of an infinite number of uses, where entangled between multiple applications is allowed in general.\comment{Why is this called the same thing as before but defined differently and using a different symbol for the metric?}
...
\end{align}
\comment{The symbol $\backslash >$ doesn't work. Please correct this.}
...
\comment{This is an excellent idea to have a table summarising the different channel capacity measures!! Good thinking! Perhaps also have a column describing the interpretation of the measure, in English rather than in maths.}

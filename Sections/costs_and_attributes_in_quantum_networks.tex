...
Suppose we have a random variable $X$, and a particular measurement outcome $x$ occurs with probability $p(x)$, the entropy of $X$ is given by,
\begin{align}\index{Shannon entropy}
H(X) = -\sum_x p_x\log_2(p_x).
\end{align}

The information, or entropy, plays a central role in information theory. 
The intuitive interpretation is that,it quantifies an experimenter's uncertainty about $X$
before measuring it, and his expected information gain is $H(X)$ bits upon learning the outcome. The information $H(X)$ is zero if and only if one of the probabilities
$p(x)$ is unity, with the others being zero. In this case the value of $X$ is
...
We now introduce an entropic measure of common or mutual information that two party possesses. Suppose Alice possesses the random variable $A$ and Bob has the random variable $B$, the \textit{mutual information} specifies the number of bits in common between the two distributions. Equivalently, it is the maximum number of bits that one party can learn about the other. For two classical distributions, this is given by,
\begin{align}\index{Mutual information}
I(A;B) = H(A) + H(B) - H(A,B).
\end{align}

We can also define the classical mutual information $I_c(\rho_{AB})$ for a bipartite quantum state $\rho_{AB}$. Suppose that $\rho_{AB}$ is shared between Alice and Bob and they would like to extract maximal correlations from it. They measure using local measurement bases $\{\Lambda_A^x\}$ and $\{\Lambda_B^x\}$, which yields random varialbes $X$ and $Y$.  Their classical mutual information is given by
\begin{align}
I_c(\rho_{AB}) \equiv \max_{\{\Lambda_A^x\},\{\Lambda_B^x\}} I(X;Y).
\end{align}

Analogously, the quantum mutual information  for a bipartite state $\rho_{AB}$ is defined as
\begin{align}
I(A;B)_\rho = S(\hat\rho_A) + S(\hat\rho_B) - S(\rho_{AB}),
\end{align}
...
\begin{align}
I(A;B)_\rho = I(\hat{U}_A\hat\rho_A \hat{U}_A^\dag; \hat{U}_B\hat\rho_B \hat{U}_B^\dag),
\end{align}
...
% 
The mutual information is defined as being between a particular known pair of states. Of course, in a channel we have room 
play around with the input state and the measurement at the output. Therefore we can optimize the mutual information of a particular channel, which leads to the following definiiton:

The \textit{classical mutual information}\index{Classical channel capacity} of channel $\mathcal{E}$ is defined as,
\begin{align}\index{mutinfoclassical}
I(\mathcal{E}) = \max_{p(x)} I(X;Y)
% 
\end{align}
with the intuitive interpretation as the maximum mutual information between input and output states that can be achieved for the channel.  For a noisy channel, if the input contains product states (i.e. entanglement between input states is not allowed), the classical capacity can be expressed as
\begin{align}
\chi(\mathcal{E}) = \max_{p_i,\rho_i} S(\rho) - \sum_i p_i S(\rho_i) \label{eq:holevoinfo}
\end{align}
\noindent which is known as the Holevo bound.


And the \textit{classical channel capacity} of $\mathcal{N}$ is defined as
\begin{align}\index{classical!Channels!Capacity}
\mathcal{C}(\mathcal{E}) = \lim_{n \rightarrow \infty} \frac{1}{n} \mathcal{E}^{\otimes n},
\end{align}
(Note: this definition of `capacity' is not to be confused with that which we later refer to in flow networks).
...
\begin{align}
I(A\>B) \equiv H(B)_\rho - H(AB)_\rho
\end{align}
\noindent which we recognize as the negative of conditional quantum entropy $H(A|B) = H(AB)_\rho - H(B)$. The quantity $H(A|B)$ can be negative is a signature of the fundamental difference between the classical and the quantum world.
...
The \textit{quantum channel capacity} is then defined as
\begin{align}
Q(\mathcal{E}) = \lim_{n \rightarrow \infty} \frac{1}{n} \mathcal{I}_Q(\mathcal{E}^{\otimes n})
\label{eq:quantcohinfo}
\end{align}

\textit{The most important distinction between quantum mutual information and quantum coherent information is that, the 
mutual information is always additive, but the quantum coherent information is not.}
Analytic solutions to Eq.~\eqref{eq:quantcohinfo} are notoriously difficult to calculate. However, once net dephasing or depolarisation rates have been calculated across a route, the \textit{single-use} quantities can be calculated, which can serve as a lower bound to the ultimately achievable rates, if a number representing \textit{cost}, rather than an analytic solution, is all we need.



% A summary of the different measure of classical and quantum capacities are given in Table \ref{tab:capacitie}.
% \begin{widetext}
% \begin{table}[hbt]
% \begin{tabular}{ |c | c | c| c| } \hline
% Capacity & Type of information & Correlation measure &Capacity formula  \\ \hline
% Classical &Classical information &Holevo information &Eq.~\eqref{eq:holevoinfo}       \\
% % Entanglement Assisted Classical & Classical information & Quantum mutual information &Bennett-Shor-Smolin-Thapliyal formula \\
% Quantum &Quantum information &Quantum Coherent Information &Eq\eqref{eq:quantcohinfo} \\ \hline
% \end{tabular}
% \caption{\label{tab:capacitie} Measure of classical and quantum channel capacities.}
% \end{table}
% \end{widetext}
...
For a more in-depth discussion of channel capacities 

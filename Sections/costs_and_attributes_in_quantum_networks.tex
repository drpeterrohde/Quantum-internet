...
\mathcal{C}(\mathcal{E}) = \lim_{n \rightarrow \infty} \frac{1}{n} I(\mathcal{E}^{\otimes n}),
\end{align}
\comment{Added missing entropy/capacity measure in here that needs to act on E. Correct?}
which can be interpreted as the maximum bitrate of the channel, per use of the channel, in the limit of an infinite number of uses.
(Note: this definition of `capacity' is not to be confused with that which we later refer to in flow networks).\comment{Why is this called the same thing as before but defined differently and using a different symbol for the metric?}

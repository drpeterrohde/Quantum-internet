...
Suppose $\hat\rho_{A,B}$ is shared between Alice and Bob, from which they would like to extract maximal correlations (i.e joint information). They measure using local measurement bases $\{\Lambda_A^x\}$ and $\{\Lambda_B^x\}$, yielding random variables $A$ and $B$. Recall that their quantum mutual information of the channel is defined as,
...
%\subparagraph{Classical channel capacity}

The \textit{classical channel capacity} of a quantum channel $\mathcal{E}$ is defined as
...
\end{align}
which is known as the Holevo bound\index{Holevo bound}.
...
\subparagraph{Superactivation}\index{Superactivation}

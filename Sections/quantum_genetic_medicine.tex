...
\comment{Add equations for what oracles do}
...
Using standard quantum chemistry techniques, one can construct accurate Hamiltonians\index{Molecular Hamiltonians} describing the evolution of molecular systems at the quantum level. Efficiently implementing such simulations can then be performed on quantum computers using standard, efficient Hamiltonian simulation quantum algorithms (Sec.~\ref{sec:quantum_sim_alg})\index{Hamiltonian simulation}.
...
\end{align}
where $m_i$ is mass, and $\vec{r}$ is the position vector. And the dominant two-body terms are Coulomb (electrostatic) interactions\index{Coulomb interaction},
\begin{align}
...
\begin{figure}[!htb]
	\includegraphics[width=0.55\columnwidth]{genetic_medicine_pipeline}

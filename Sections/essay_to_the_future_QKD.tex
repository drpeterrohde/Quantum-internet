...
Classical digital signature is a cryptographic primitive whose purpose is to ensure that a message's 
\begin{enumerate}
...
\item Signing algorithm: the sender chooses a message $M$ and uses the private key $sk$ to generate a signature, $\sigma = \sigma(M)$. She sends the pair ($M,\sigma(M)$) to the desired receiver(s)

...
An individual user of a quantum computer operating on their own, observes computational power characterised by the appropriate computational scaling function acting on the number qubits in their possession, $f_\mathrm{sc}(n)$. This stipulates the utility payoff for that individual, allowing for trivial  construction (and efficient mathematical representation) of multi-player payoff matrices simply by characterising single-player payoffs independently.
...
c_X &= \frac{n_X}{n_X+n_Y}\cdot f^X_\mathrm{sc}(n_X+n_Y),\nonumber\\
c_Y &= \frac{n_Y}{n_X+n_Y}\cdot f^Y_\mathrm{sc}(n_X+n_Y),
\end{align}
...
\begin{itemize}
	...
	\item The asymmetry in the distribution of qubits between them,
		...
		\end{align}
	\item The respective algorithms to which the computational resources are being applied by each player, which influence the subjective scaling functions, $f^i_\mathrm{sc}$, independently.
\end{itemize}

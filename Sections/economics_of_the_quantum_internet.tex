...
We begin by defining the ratio $Q_t$ between the economic return from executing computations, and the cost of the investment into physical hardware performing them,
\begin{align}
Q_t=\frac{n_\text{return}(t)}{n_\text{invest}(t)}.
\end{align}
...
\begin{align} \label{sec:gain_eq_invest}
	n_\text{return}(t) \gtrapprox n_\text{invest}(t).
\end{align}
...
\end{align}
based on the network growth postulate (Pos.~\ref{post:net_growth})\index{Network growth postulate} and employing a combined computational scaling function\index{Combined computational scaling functions} (Def.~\ref{def:comb_sc_func}). The dollar-value of current net computational power of the entire network scales as,
\begin{align}
	n_\text{gain}(t) &= N_t L_t \cdot n_\text{FLOPS}(t) \nonumber \\
	&= N_0 {\gamma_N}^t L_t\cdot f_\text{sc}(N_0 {\gamma_N}^t).
\end{align}
And the present-day dollar-value of the physical hardware is given by the network value (Pos.~\ref{post:network_value}),
\begin{align}
	n_\text{cost}(t) &= P_t.
\end{align}
...
This implies,
\begin{postulate}[Spot price of computation]\index{Spot price of computation} The present-day (\mbox{$t=0$}) spot price of a computation is, measured in FLOPS per qubit,
\begin{align}
...
D_t &= L_t c_n(t) \nonumber \\
&= n \frac{C_0{\gamma_C}^t}{N_0{\gamma_N}^t}.
\end{align}
\end{definition}
Up to normalisation this is equivalent to the network value (Pos.~\ref{post:network_value}). Note that the spot price, $D_0$, is the dividend at time \mbox{$t=0$}.
...
F_T = D_0 e^{rT} - \sum_{t=1}^T D_t e^{r(T-t)},
\end{align}
where,
\begin{itemize}
	\item $F_T$ is the forward price at time $T$.\index{Forward price}
	\item $D_0$ is the spot price at time \mbox{$t=0$} (i.e present-day value).
...
\begin{align}
F_T &= D_0 e^{rT} - \sum_{t=1}^T D_t e^{r(T-t)} \nonumber \\
&= n C_0 \left[e^{rT} + \frac{\gamma_C(e^{rT}-{\gamma_C}^T{\gamma_N}^{-T})}{N_0(\gamma_C - e^r\gamma_N)}\right].
\end{align}
\end{definition}

Note that in the limit of \mbox{$T\to 0$} this reduces to \mbox{$F_0 = D_0 = nC_0$}, which is simply the spot price of the asset.

\begin{figure}[!htb]
\includegraphics[width=0.4\textwidth]{forward_cont_pricing_mod}
\caption{Forward contract price on a single physical qubit to be delivered at time $T$ in the future. Units are \mbox{$C_0=1$}.\comment{Redo fig, outdated}}\label{fig:forward_cont_pricing_mod}
\end{figure}
...
\begin{align}
R_T = \frac{c_\text{profit}(T)}{c_\text{cost}(0)}.
\end{align}

Let the cost of the asset be the current spot price,
\begin{align}
c_\text{cost}(0) &= C_0.
\end{align}
Let the profit be the accumulated dividends, paid up until time $T$,
\begin{align}\index{Profit}
c_\text{profit}(T) &= \sum_{t=0}^T D_t e^{-rt}. \nonumber \\
\end{align}
Then the RoI is given by,
\begin{align}
	R_T = \frac{\sum_{t=0}^T D_t e^{-rt}}{C_0}.
\end{align}
...
\begin{align}
	R_T &= \frac{\sum_{t=0}^T D_t e^{-rt}}{C_0}\nonumber \\
	&=\frac{e^{-rT}\gamma_C\left(\frac{\gamma_C}{\gamma_N}
	\right)^T - e^r\gamma_N}{N_0(\gamma_C-\gamma_N)}.
\end{align}
\end{definition}
...
\] & \[n\] & \[n\cdot{n_\text{global}}^{p-1}\] & \[\frac{n e^{n_\text{global}}}{n_\text{global}}\]\\
	...
	\hline
	Forward contract price (Sec.~\ref{sec:for_contr}) & \[F_T\]=??? & ??? & ??? & ??? \\
	\hline
	Return on investment (Sec.~\ref{sec:ROI}) & \[R_T\]=??? & ??? & ??? & ???\\
	\hline

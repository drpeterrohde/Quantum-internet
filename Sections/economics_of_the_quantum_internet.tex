...
Suppose we have a classical network comprising two computers with equal processing power, one held by Alice and the other by Bob. One could network these computers into a cluster to create a more powerful virtual computer of approximately twice the power of the individual computers. When utilising them, one option is for Alice and Bob to each use their individual computers in isolation for a given duration. The other  is for them to cluster them and agree to time-share the virtual computer, each using the cluster for half the time. Which is the better outcome for Alice and Bob?

In the case of classical computing there is clearly no difference between using the computers in isolation or networking them and time-sharing their joint compute power -- one either uses a computer twice as powerful for half the amount of time, or half as powerful for twice the amount of time.

For quantum computers this is no longer the case, since the computational power of quantum computers in general grows super-linearly with the number of qubits. That is, the joint computational power of unified quantum computers is more than the sum of the parts.

Therefore, in the quantum context it makes more sense to unify computational assets into a larger virtual computer and time-share its joint computational power than to utilise the assets in isolation.

For this reason, henceforth we will assume an environment in which owners of quantum hardware network and unify their computational power, sharing the virtual computer's power between them.

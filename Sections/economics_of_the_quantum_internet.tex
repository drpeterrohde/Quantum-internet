...
In Sec.~\ref{sec:quantcoin} we present a high-level argument that forward contracts in quantum computer runtime could form the basis of a sound currency, backed by the economy's desire for computation.

%
% QuantCoin - A Quantum Computation-Backed Cryptocurrency
%

\subsection{QuantCoin\texttrademark --- A quantum computation-backed cryptocurrency}

\comment{To do}

\comment{Thus with this sequence the execution of the QC is equivalent to mining, where the verification protocol acts as the proof of work. The protocol acts as a currency converter, allowing Alice to convert dollars into coins.

In an efficient market we have the relationship:

(Dollar value of a QuantCoin) + (cash paid by Alice for execution) = (Dollar value of cost associated with physically executing the protocol on the QC by Bob).

Alternately we could replace dollars with any other asset or existing QuantCoins. In the latter case one would have relationships like:

(value of QuantCoin) =  (Dollar value of cost associated with executing the protocol on the QC by Bob) / 2.

This represents expansion in the money supply associated with each successful computation.}

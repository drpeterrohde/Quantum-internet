...
\captionspacefig \caption{\comment{To do}}\label{fig:game_theory_1}
\end{figure}
...
The final strategy available to the firms is to consolidate their resources into a distributed cloud computing environment (Sec.~\ref{sec:dist_comp}), where the limitations of a single system are lifted, allowing for even better resource-sharing and improvement beyond what a single system solution can offer. In this scenario, both providers will be able to collaborate such that they can meet their individual computational requirements, without having to fully build independent systems like that in the licensing strategy. Importantly though, while there may be a small loss in utility for one of the parties compared to the licensing strategy, the cooperation allows an overall higher level of utility to be achieved jointly, creating a Nash equilibrium\index{Nash equilibrium} at this point, $C$.

This can be described in the utility payoff matrix outlined in Tab.~\ref{tab:game_theory_1}, where `$\leftarrow/\rightarrow$' indicates the licensing (from/to) strategy, `+' indicates the cloud computing cooperation strategy, and the off-diagonal combinations are the status quo, indicating no cooperation.
...
\end{tabular}
\captionspacetab \caption{\comment{To do}}\label{tab:game_theory_1}
\end{table}

It's important to note that these values are totally arbitrary in nature, and do not have a `real life' interpretation, other than understanding the preferencing of the described strategies. For initial simplicity, the matrix assumes that $A$ and $B$ derive identical utility values from the strategies, resulting in a symmetrical representation of the choices in Fig.~\ref{fig:game_theory_2} around the diagonal axis.

Translating the utility payoff matrix into a graphical representation yields Fig.~\ref{fig:game_theory_2}.

\begin{figure}[!htbp]
\includegraphics[clip=true, width=0.475\textwidth]{game_theory_2}
\captionspacefig \caption{\comment{To do}}\label{fig:game_theory_2}
\end{figure}
...
\multirow{4}{*}{$Y$} & Strategy         & $X\rightarrow Y$ & $X+Y$       & $X\leftarrow Y$ \\ \cline{2-5} 
                     ...
                     & $X\leftarrow Y$  & $(1,1)$          & $(1,1)$     & $(1.5,3)$       \\ \hline
\end{tabular}
\captionspacetab \caption{\comment{To do}}\label{tab:game_theory_2}
\end{table}
...
\captionspacefig \caption{\comment{To do}}\label{fig:game_theory_3}
\end{figure}
...
\captionspacefig \caption{\comment{To do}}\label{fig:game_theory_4}
\end{figure}
...
\captionspacefig \caption{\comment{To do}}\label{fig:game_theory_5}
\end{figure}
...
\multirow{4}{*}{$Y$} & Strategy         & $X\rightarrow Y$ & $X+Y$       & $X\leftarrow Y$ \\ \cline{2-5} 
                     ...
                     & $X\leftarrow Y$  & $(1,1)$          & $(1,1)$     & $(1.5,3)$       \\ \hline
\end{tabular}
\captionspacetab \caption{\comment{To do}}\label{tab:game_theory_3}
\end{table}
...
\captionspacefig \caption{\comment{To do}}\label{fig:game_theory_6}
\end{figure}
...
\begin{itemize}
	...
	\item Competing cartels develop, where for external reasons (e.g. political, geographical, ideological, strategic), there is a benefit in cooperating with just a few players, and acting as a single supplier in direct competition with other cartels.
\end{itemize}

...
\begin{enumerate}
	\item Alice possesses the raw $data$ she would like computed upon, as well as a $reward$ to pay for its processing (in arbitrary units). This could be in the form of QuantCoins, dollars, or other assets of monetary value.
	...
	\item If it is valid, the $blockchain$ cryptographically hashes the data and proof, encapsulating it into a QuantCoin\texttrademark\,token, which is given to Bob,
	\begin{align}
		&token = SHA256(encryptedInput + encryptedOutput + proof)\nonumber\\
		&Bob.receive(token)
	\end{align}
\end{enumerate}}}}
\caption{A rough high-level outline for how a quantum computation-backed cryptocurrency might be implemented\footnote{Note this is merely a highly simplified toy model in which all computations are of equal value, which will typically not be the case in the real world. A consumer-ready implementation would have to handle these and other additional details. However, we expect these finer details oughtn't alter the overall flavour of the protocol as described.}. We have abstracted away the underlying Blockchain protocol, interfacing with it using of a high-level API, so as to abstract away the underlying Blockchain technology, which is liable to change substantially in the coming years -- it might even be a natively quantum Blockchain. We similarly call upon verification subroutines using a high-level implementation-dependent API.} \label{alg:quant_coin}
\end{table}
...
What relationships characterise the value of QuantCoins? First, in a perfectly efficient market we have,

...
Suppose we wish to search over the $N$-bit space $x$, to find a satisfying solution to some oracle function \mbox{$f(x)=1$}. Let there be $M$ nodes available for computation, where for simplicity we assume $M$ is a power of 2. We designate each of the nodes a $\log_2 M$-bit identification number\index{Identification numbers},
\begin{align}
	y=[0,M-1].
\end{align}

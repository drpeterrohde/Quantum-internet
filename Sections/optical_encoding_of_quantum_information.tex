...
Coherent state encoding can be regarded as encoding via the displacement operator, which implements translations in phase-space. Alternately, we can encode via the squeezing operator, which implements dilations about an axis in phase-space. Squeezing the vacuum state along the $\hat{x}$ or $\hat{p}$ directions yields two states that are approximately orthogonal for large squeezing amplitudes, as shown in Fig.~\ref{fig:squeezed_state_encoding}. Thus, with sufficient squeezing they can be used as a basis for approximately encoding a qubit.

\comment{Show maths}

\begin{figure}[!htb]
\includegraphics[width=0.4\textwidth]{squeezed_state_encoding}

...
\if 2\pubmode
	\begin{figure}[!htbp]
	\includegraphics[width=0.15\textwidth]{photonic_qubit_encodings_long}
	\captionspace \caption{Three approaches to encoding a single qubit using a single photon, via a superposition across two spatial ($A$ and $B$), polarisation ($V$ and $H$) or temporal ($t$ and \mbox{$t+\tau$}) modes.} \label{fig:photonic_qubit_encodings}
	\end{figure}
\else
	\begin{figure*}[!htbp]
	\includegraphics[width=0.5\textwidth]{photonic_qubit_encodings}
	\captionspace \caption{Three approaches to encoding a single qubit using a single photon, via a superposition across two spatial ($A$ and $B$), polarisation ($V$ and $H$) or temporal ($t$ and \mbox{$t+\tau$}) modes.} \label{fig:photonic_qubit_encodings}
	\end{figure*}
\fi
...
\if 2\pubmode
	\begin{figure}[!htbp]
	\includegraphics[width=0.475\textwidth]{TEM_cylinder}\\
	\includegraphics[width=0.475\textwidth]{TEM_rectangular}
	\captionspace \caption{TEM$_{m,n}$ modes for cylindrical (top) and rectangular (bottom) mode profiles.}\index{Transverse electro-magnetic (TEM) modes}\label{fig:TEM_modes}
	\end{figure}
\else
	\begin{figure*}[!htbp]
	\includegraphics[width=0.49\textwidth]{TEM_cylinder}
	\includegraphics[width=0.49\textwidth]{TEM_rectangular}
	\captionspace \caption{TEM$_{m,n}$ modes for cylindrical (left) and rectangular (right) mode profiles.}\index{Transverse electro-magnetic (TEM) modes}\label{fig:TEM_modes}
	\end{figure*}
\fi
...
\captionspace \caption{Time-bin encoding on an $n$-level quantum system. Time-bin are temporally localised with envelopes $\xi_j$, orthogonal to all others, different only via temporal displacements. The temporal separation between each consecutive time-bin is given by $\tau$.}\label{fig:time_bin_encoding}	
\end{figure}
...
\captionspace \caption{Phase-space representation of a coherent state with complex amplitudes $\pm\alpha$. For sufficiently large amplitude one can approximate orthogonality, enabling a qubit encoding.} \label{fig:coherent_state_encoding}	
\end{figure}

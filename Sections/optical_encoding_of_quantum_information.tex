...
\begin{figure}[htpb]
	\includegraphics[width=0.475\textwidth]{hermite_basis}
	\caption{The family of Hermite functions, $H_n(x)$, are an example of an orthonormal basis of continuous functions that could be utilised in the spatio-temporal encoding of qudits into photonic wave-packets. Shown here are \mbox{$n=0,1,2,3$}.}\index{Hermite functions}\label{fig:hermite_basis}
\end{figure}

\subsubsection{Transverse electro-magnetic modes}\index{Transverse electro-magnetic modes}

Of particular interest are transverse electro-magnetic (TEM) modes, which are the two-dimensional eigenfunctions of the electro-magnetic field in the transverse direction of a light field. These are given by modulations of Hermite or Laguerre polynomials\index{Laguerre polynomials}, depending on the geometry of the medium through which the light is propagating (e.g optical fibre, free-space, wave-guides), which effectively impose boundary conditions on the electro-magnetic field.

The TEM modes are discrete, and denoted TEM$_{mn}$, where \mbox{$m,n\in \mathbb{Z}_+$}. TEM modes can be prepared and manipulated using holograms\index{Holograms}. Examples are shown in Fig.~\ref{fig:TEM_modes}.

\begin{figure*}[htpb]
\includegraphics[width=0.49\textwidth]{TEM_cylinder}
\includegraphics[width=0.49\textwidth]{TEM_rectangular}
\caption{TEM$_{mn}$ modes for cylindrical (left) and rectangular (right) mode profiles.}\index{Transverse electro-magnetic modes}\label{fig:TEM_modes}
\end{figure*}
...
\begin{figure}[htpb]
\includegraphics[width=0.25\textwidth]{time_bin_encoding}
\caption{Time-bin encoding on an $n$-level quantum system. Time-bin are temporally localised with envelopes $\xi_j$, orthogonal to all others, different only via temporal displacements. The temporal separation between each consecutive time-bin is given by $\tau$.}\label{fig:time_bin_encoding}	
\end{figure}

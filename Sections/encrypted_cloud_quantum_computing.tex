...
A homomorphic encryption scheme is made up of four algorithms: a key generation algorithm, an encryption algorithm, an evaluation algorithm, and a decryption algorithm.
%
% Algorithm:Keygen
%
%\begin{algorithm}
%\caption{Key Generation algorithm}\label{euclid}
%\begin{algorithmic}[1]
%\Procedure{KeyGen(R, ${\bf B}_I$)}{}
%\BState \textbf{Input:} ring $R$, basis ${\bf B}_I$ of 
%a ideal $I\subset R$
%\BState \textbf{Output:} pk=($R$, ${\bf B}_I$, 
%$B_J^{\rm pk}$, {\sc Samp}), $sk={\bf B}_J^{\rm sk}$
%\State $({\bf B}_J^{\rm sk},{\bf B}_J^{\rm pk}) \gets 
%\textit{\sc IdealGen}(R, {\bf B}_I) $
%\BState \emph{top}:
...
%\State \textbf{close};
%\EndIf
%\State $i \gets i+\max(\textit{delta}
%_1(\textit{string}(i)),\textit{delta}_2(j))$.
%\EndProcedure
%\end{algorithmic}
%\end{algorithm}

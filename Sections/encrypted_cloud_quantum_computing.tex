...
\sihui{The specifics of these algorithm vary from scheme to scheme, and as in the case of FHE, usually contains sub-algorithms within them.}

%The Algorithm:Keygen

\begin{algorithm}
\caption{Key Generation algorithm}\label{euclid}
\begin{algorithmic}[1]
\Procedure{KeyGen($\lambda$)}{}
\BState \textbf{Input:} Security parameter $\lambda$
\BState \textbf{Output:} public key $pk$, secret key $sk$
\BState \emph{top}:

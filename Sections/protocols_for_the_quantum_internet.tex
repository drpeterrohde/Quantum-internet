...
\begin{align}
\mathcal{E}(\hat\rho) = \sum_{n=0}^\infty P(m|n) \hat{E}_n\hat\rho\hat{E}_n,	
\end{align}
where \mbox{$\hat{E}_n=\ket{n}\bra{n}$} are the Kraus operators\index{Kraus operators}.
...
\begin{align}
	P(n_\text{meas}=n_\text{inc}) = \binom{m}{n_\text{inc}}\left(\frac{\eta}{m}\right)^{n_\text{inc}},
\end{align}
not accounting for other lesser errors such as dark-counts.

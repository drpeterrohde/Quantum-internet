...
\captionspacefig \caption{Quantum clock synchronisation schemes. (top) The proposal by Jozsa \& Dowling \textit{et al.} where a singlet state\index{Bell states} is shared and measured in an ensemble of qubits \cite{bib:jozsa00}. (bottom) The proposal by Lukin \& Ye \textit{et al.} where a GHZ state\index{GHZ states} is distributed between satellites to measure the frequency drift at the Heisenberg limit \cite{bib:komar14}.}
\label{fig:space_3}
\end{figure*}
...
\begin{figure}[!htbp]
	...
	\captionspacefig \caption{An array of quantum-enabled telescopes, paired with one another via Bell-pairs, distributed over a quantum network.}\label{fig:telescope_array}\index{Telescope arrays}
\end{figure}

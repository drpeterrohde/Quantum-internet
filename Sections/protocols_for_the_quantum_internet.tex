...
\else
	...
	\includegraphics[clip=true, width=0.475\textwidth]{W_state_QEC_fidelity}
\fi
\caption{Heralding probability and associated error-corrected fidelity for a single qubit stored in a W-state quantum error correction circuit with decoherence times $T_1$ (amplitude damping) and $T_2$ (dephasing)\index{T$_1$-time}\index{T$_2$-time}. We have chosen $\theta=?$
\comment{Get vector versions}}\label{fig:W_state_QEC_fidelity}
\end{figure}

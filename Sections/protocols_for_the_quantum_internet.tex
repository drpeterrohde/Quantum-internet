...
\begin{figure}[!htbp]
	\includegraphics[width=0.475\textwidth]{GHZ_LO_prep}
...
\begin{figure}[!htb]
\includegraphics[width=0.475\textwidth]{W_state_LO_prep}
...
\captionspace \caption{A superconducting photo-detector. A superconductor is held at just below its critical temperature. The absorption of a photon is sufficient to heat it above the critical temperature, yielding a detectable change in conductance across the device.} \label{fig:super_det}
\end{figure}
...
\begin{figure}[!htbp]
	\includegraphics[width=0.475\textwidth]{multiplexed_detector}
...
\end{align}
\end{mdframed}
\captionspace \caption{Quantum state teleportation of a single qubit.} \label{alg:state_teleport}
\end{table}
...
\end{align}
\end{mdframed}
\captionspace \caption{Teleporting a CNOT gate onto a 2-qubit state.} \label{alg:gate_teleport}
\end{table}
...
\end{align}
\end{mdframed}
\captionspace \caption{Entanglement swapping protocol between two parties. Two Bell pairs held locally by two users, \mbox{$\ket{\Phi^+}_{A_1,A_2}\ket{\Phi^+}_{B_1,B_2}$}, are converted to a single Bell pair shared between the users, $\ket{\Phi^+}_{A_2,B_2}$.} \label{alg:ent_swap}
\end{table}
...
\end{mdframed}
\captionspace \caption{Superdense coding protocol for communicating two classical bits via transmission of a single qubit. The protocol requires the two parties share a Bell pair as a resource, provided by a third party.} \label{alg:superdense}
\end{table}
...
\end{mdframed}
\captionspace \caption{Algorithm for performing quantum clock synchronisation between two parties using shared Bell-pairs and classical communication.\comment{Check this for errors!}} \label{alg:clock_sync}
\end{table}
...
\begin{figure}[!htbp]
	\includegraphics[width=0.475\textwidth]{telescope_array}
	\captionspace \caption{An array of quantum-enabled telescopes, paired with one another via Bell-pairs, distributed over a quantum network.}\label{fig:telescope_array}\index{Telescope arrays}
\end{figure}

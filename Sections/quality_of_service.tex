...
\begin{figure}[!htb]
	...
	\caption{Implementation of logical Pauli-$\hat{X}$ (\comment{which colour?}) and -$\hat{Z}$ (\comment{which colour?}) operations on a toric code qubit via the application of a chain of local Paulis. The chains must circumvent the torus around either axis. Any path topologically equivalent to such a circumvention of the torus is sufficient, enabling paths to bypass defects in the lattice.} \label{fig:toric_code_paulis}
\end{figure}
...
\comment{Toric code -- shift this to surface code section}

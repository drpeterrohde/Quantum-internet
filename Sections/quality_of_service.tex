...
\end{align}
which corresponds to a bit-flip error model (Sec.~\ref{sec:dephasing_error}). Alice and Bob then perform their entanglement purification protocol -- operations to reduce them to a single Bell pair of higher fidelity,
\begin{align}
...
The purification process $\mathcal{E}_\text{purif}$ is realised very simply by Alice and Bob both performing Bell projections (Sec.~\ref{sec:bell_proj}) on their two qubits, classically communicating their measurement outcomes, and performing appropriate local corrections. This is best illustrated by example.

For this model, we will assume our Bell projections destroy the first of their two input qubits, while the second one remains as the output. Then two such projections project a four-qubit state down to a two-qubit state as desired.
...
\end{align}
Now upon inspection it is evident that if a Bell projection between Alice's two qubits (1 and 3) yields the $\ket{\Phi^+}$ outcome, Bob's projection (2 and 4) must also yield $\ket{\Phi^+}$. Similarly, whatever Bell outcome Alice measures, Bob will measure the same outcome. Upon classically communicating that they measured consistent outcomes, they are satisfied the final state is error-free, and it is.
...
\end{align}
Upon performing the Bell measurements as before, suppose Alice measures the $\ket{\Psi^+}$ outcome. Now Bob must measure the $\ket{\Phi^+}$ outcome, and a comparison of measurement outcomes diagnoses this single bit-flip error, which may subsequently be corrected with an additional bit-flip.
...
\end{align}
Upon performing the Bell projections, both Alice and Bob are guaranteed to measure the same outcome, as per the error-free case. However this isn't error-free at all since the remaining state is $\ket{\Psi^+}$ not $\ket{\Phi^+}$. That is, the bit-flip error has permeated through to the corrected state.
...
Similar reasoning as above can be employed to correct against phase-flip errors, and depolarising errors more generally.

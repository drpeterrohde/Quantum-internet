...
\section{Quality of service}\index{Quantum error correction}\index{Quality of service}\label{sec:QOS_chap}
...
More concretely, we distribute two Bell pairs of fidelity $F$ between Alice and Bob. Let us assume a dephasing error model in this instance, in which case the imperfect Bell pairs are of the form,
\begin{align}
\hat\rho^F_{A,B} = F\ket{\Phi^+}\bra{\Phi^+} + (1-F)\hat{X}_1\ket{\Phi^+}\bra{\Phi^+}\hat{X}_1,	
\end{align}
which corresponds to a bit-flip error model (Sec.~\ref{sec:dephasing_error}). That is, we have a mixture of,
\begin{align}
...
The purification process $\mathcal{E}_\text{purif}$ is realised very simply by Alice and Bob both performing parity measurements\index{Bell measurement} (see Sec.~\ref{sec:bell_proj} for an optical implementation) on their two qubits, whose measurement outcomes indicate whether two qubits were equal ($+$) or different ($-$). These are easily implemented using CNOT gates, using the construction shown in Fig.~\ref{fig:parity_meas}. They complete the protocol by classically communicating their measurement outcomes, and performing appropriate local corrections. This is best illustrated by example.
...
\begin{figure}[!htb]
	...
	\end{align}
	\caption{Construction of a parity measurement circuit using a single CNOT gate and a single-qubit measurement. The measurement outcome indicates whether the two input qubits were the same (\mbox{$\ket{0}\ket{0}$}, \mbox{$\ket{1}\ket{1}$}) or different (\mbox{$\ket{0}\ket{1}$}, \mbox{$\ket{1}\ket{0}$}).}\label{fig:parity_meas}
\end{figure}

For this model, we will assume our parity projections destroy the first of their two input qubits, while the second one remains as the output. Then two such projections project a 4-qubit state down to a 2-qubit state as desired.

Suppose there was no error. Then the input state was,
\begin{align}
\ket\psi_\text{in} &= \ket{\Phi^+}\bra{\Phi^+} \otimes \ket{\Phi^+}\bra{\Phi^+}\\
&= \frac{1}{2}(\ket{0}_1\ket{0}_2+\ket{1}_1\ket{1}_2)(\ket{0}_3\ket{0}_4+\ket{1}_3\ket{1}_4).\nonumber
\end{align}
Now upon inspection it is evident that if a parity projection between Alice's two qubits (1 and 3) yields the $+$ outcome, Bob's projection (2 and 4) must also yield $+$. Similarly, for the $-$ outcome. Upon classically communicating that they measured consistent outcomes, they are satisfied the final state is error-free, and it is, leaving them with a perfect $\ket{\Phi^+}$ state.

Now suppose there was a bit-flip error on the first qubit of the first pair. Now the input state is,
\begin{align}
\ket\psi_\text{in} &= \hat{X}_1 \ket{\Phi^+}\bra{\Phi^+}\hat{X}_1 \otimes \ket{\Phi^+}\bra{\Phi^+}\\
&= \frac{1}{2}(\ket{1}_1\ket{0}_2+\ket{0}_1\ket{1}_2)(\ket{0}_3\ket{0}_4+\ket{1}_3\ket{1}_4).\nonumber
\end{align}
Upon performing the parity measurements as before, suppose Alice measures the $+$ outcome. Now Bob must measure the $-$ outcome, and a comparison of measurement outcomes diagnoses this single bit-flip error, which may subsequently be corrected with an additional bit-flip.

In the above two scenarios (no error, or exactly one error), the state is correctable to a perfect Bell pair. What if there are two errors? Consider the input state with bit-flips affecting both Bell pairs. Then,
\begin{align}
\ket\psi_\text{in} &= \hat{X}_1 \ket{\Phi^+}\bra{\Phi^+}\hat{X}_1 \otimes \hat{X}_3\ket{\Phi^+}\bra{\Phi^+}\hat{X}_3\\
&= \frac{1}{2}(\ket{1}_1\ket{0}_2+\ket{0}_1\ket{1}_2)(\ket{1}_3\ket{0}_4+\ket{0}_3\ket{1}_4).\nonumber
\end{align}
Upon performing the parity measurement, both Alice and Bob are guaranteed to measure the same outcome, as per the error-free case. However this isn't error-free at all since the remaining state is $\ket{\Psi^+}$ not $\ket{\Phi^+}$. That is, the bit-flip error has permeated through to the `corrected' state.

Following this example it is evident that our rudimentary entanglement purification protocol can correct at most one bit-flip error. For this reason a mixed state is only correctable if \mbox{$F>1/2$} to begin with. Otherwise errors are more likely to permeate than be corrected, thereby reducing output fidelity.

The full circuit for this entanglement purification protocol is shown in Fig.~\ref{fig:ent_purif_circ}.
...
Similar reasoning as above can be employed to correct against phase-flip errors by operating in a Hadamard-rotated basis.

In Sec.~\ref{sec:ent_purif} we demonstrate a simple linear optics setup for implementing this, that was subject to experimental demonstration.
...
\begin{figure*}[!htb]
	\includegraphics[width=0.47\textwidth]{torus}
...
\begin{figure}[!htb]
	\includegraphics[width=0.47\textwidth]{torus_paulis}
	\caption{Implementation of logical Pauli-$\hat{X}$ (\comment{which colour?}) and -$\hat{Z}$ (\comment{which colour?}) operations on a toric code qubit via the application of a chain of local Paulis. The chains must circumnavigate the torus around either axis. Any path topologically equivalent to such a circumnavigation of the torus is sufficient, enabling paths to bypass defects in the lattice.} \label{fig:toric_code_paulis}
\end{figure}
...
Note that the protocol doesn't require that the $M$ instances of the unitary be held locally. They operate independently in parallel and could therefore be distributed, with the independent copies held by entirely different parties in distant locations.\index{Distributed computation}\index{Parallelisation}
...
In \cite{RohdeHaselgrove} a derivation of a generalisation of the quantum Hamming bound\index{Quantum Hamming bound} was presented, where the number of located and unlocated errors were treated as independent parameters. They found that for non-degenerate codes the Hamming bound allows exactly twice as many located as unlocated errors. Specifically, for $t_l$ located errors and $t_u$ unlocated errors in an $n$-qubit non-degenerate code encoding $k$ logical qubits, QEC is possible provided that the number of errors satisfied,
\begin{align}
\sum_{i=0}^{t_u + \lfloor t_l/2 \rfloor} \binom{n}{i}3^i 2^k \leq 2^n.
\end{align}
\comment{Explain Hamming bound, potentially in footnote. Explain what degenerate and non-degenerate codes are.}

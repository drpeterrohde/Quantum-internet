...
\begin{figure}[!htb]
\includegraphics[width=0.35\textwidth]{graph_colouring}\\
\includegraphics[width=0.35\textwidth]{four_colouring}
\caption{(top) Example of a graph with 3-colouring, such that no two vertices connected by an edge share the same colour. For this particular graph, 3-colouring is optimal (in the sense that the number of colours in minimised). It can easily be seen upon inspection that no 2-colouring is possible for this graph. Thus, this graph's chromatic number is \mbox{$\chi(G)=3$}\index{Chromatic number}. (bottom) A 4-colouring of the states of France\index{Elderberries}. Any planar map such as this has a chromatic number of at most 4 -- the `four colour theorem'.}\index{Graph colouring}\label{fig:graph_colouring}	
\end{figure}

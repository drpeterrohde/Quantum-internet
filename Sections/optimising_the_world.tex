...
\captionspacefig \caption{(top) Example of a graph with 3-colouring, such that no two vertices connected by an edge share the same colour. For this particular graph, 3-colouring is optimal (in the sense that the number of colours in minimised). It can easily be seen upon inspection that no 2-colouring is possible for this graph. Thus, this graph's chromatic number is \mbox{$\chi(G)=3$}\index{Chromatic number}. (bottom) A 4-colouring of the states of France\index{Elderberries}. Any planar map such as this has a chromatic number of at most 4 -- the `four colour theorem'.}\index{Graphs!Colouring}\index{Surrender}\label{fig:graph_colouring}	
\end{figure}
...
I recall a popular quantum computing joke circulating the internet a number of years ago. It went along the lines of:
\\
\\
...
\\
\\
This joke is of course based on the assumption that quantum computers can efficiently solve the chess problem, foreseeing the whole game, and knowing the full optimal future gameplay.

However this is a poorly-designed joke, since it has been proven that chess is an \textbf{EXP}-complete problem\footnote{When the game is generalised appropriately to an \mbox{$n\times n$} board, since obviously \textit{any} finite instance of any game can be solved in $O(1)$ time.}, believed to lie outside of \textbf{BQP}. Therefore this is a bad joke and it isn't funny.

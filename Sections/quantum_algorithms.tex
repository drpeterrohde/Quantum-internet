...
The circuit construction for the QFT is shown in Fig.~\ref{fig:QFT_circuit}.

\begin{figure}
\includegraphics[width=0.475\textwidth]{QFT}
\caption{Circuit decomposition for the quantum Fourier transform (QFT). The circuit is defined recursively, defining the $n$-qubit QFT in terms of an \mbox{$n-1$}-qubit QFT. Here \mbox{$R_\theta=\mathrm{diag}(1,e^{i\theta})$} is a generalised phase-flip gate.}\label{fig:QFT_circuit}\index{Quantum Fourier transform}
\end{figure}

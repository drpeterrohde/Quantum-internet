...
\begin{align}
\hat{\text{QFT}}_{j,k} = \frac{1}{\sqrt{N}} \omega^{(j-1)(k-1)},	
\end{align}
where,
\begin{align}
...
\begin{align}
\hat{\text{QFT}}_N = \frac{1}{\sqrt{N}} \left(\begin{matrix}
  1 & 1 & 1 & \dots & 1\\
...
\end{matrix}\right),
\end{align}
which is symmetric, \mbox{$\hat{\text{QFT}}=\hat{\text{QFT}}^\top$}. Note that all matrix elements are phases with magnitude $1/\sqrt{N}$,
\begin{align}
	|\hat{\text{QFT}}_{i,j}|=\frac{1}{\sqrt{N}}\,\,\forall\, i,j.
\end{align}

Equivalently, in terms of basis state transformations this implements the map,
\begin{align}
	\hat{\text{QFT}} \ket{j} \to \frac{1}{\sqrt{N}}\sum_{k=0}^{N-1} \omega^{jk}\ket{k},
\end{align}
...
\begin{itemize}
	...
	\item An $n$-qubit inverse quantum Fourier transform\index{Quantum Fourier transform}, $\hat{\text{QFT}}_n^\dag$ (Sec.~\ref{sec:QFT_alg}).
\end{itemize}
...
\caption{Quantum phase-estimation algorithm.} \label{alg:q_phase_est}
\end{table}
...
\subsection{Optimisation, satisfiability \& \textbf{NP}-complete problems}\index{Optimisation problems}\index{\textbf{NP} \& \textbf{NP}-complete}\index{Satisfiability problems}\label{sec:solving_NP}
...
\caption{Quantum Hamiltonian simulation algorithm.} \label{alg:ham_sim}
\end{table}
...
\begin{figure}[!htb]
\includegraphics[width=0.47\textwidth]{TDA_simplices_complex}
...
\item $\Box$
\end{enumerate}
\includegraphics[width=0.47\textwidth]{TDA_circuit}
}}}
\caption{Quantum topological data analysis algorithm for calculating Betti numbers.} \label{alg:TDA}
\end{table}

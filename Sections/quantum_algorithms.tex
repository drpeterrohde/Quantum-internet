...
\end{mdframed}
\captionspace \caption{Deutsch-Jozsa algorithm for evaluating whether the function $f(x)$ is balanced or uniform, exhibiting exponential worst case speedup compared to the best classical \textbf{BPP} algorithm\index{BPP}.} \label{alg:deutsch_jozsa}
\end{table}
...
\end{mdframed}
\captionspace \caption{Grover's algorithm for performing a quantum search over an oracle, exhibiting a quadratic quantum enhancement.} \label{alg:quant_search}
\end{table}
...
\end{mdframed}
\captionspace \caption{Quantum phase-estimation algorithm.} \label{alg:q_phase_est}
\end{table}
...
\end{mdframed}
\captionspace \caption{Quantum Hamiltonian simulation algorithm.} \label{alg:ham_sim}
\end{table}
...
\begin{figure}[!htbp]
\includegraphics[width=0.475\textwidth]{TDA_simplices_complex}
...
\else
	\includegraphics[width=0.6\textwidth]{TDA_circuit}
\fi
\end{center}
}
\end{mdframed}
\captionspace \caption{Quantum topological data analysis algorithm for calculating Betti numbers.} \label{alg:TDA}
\end{table}
...
\end{mdframed}
\captionspace \caption{The IQP sampling problem, which is believed to be a classically hard problem.} \label{alg:IQP_samp}
\end{table}

...
It's incredibly difficult to compare quantum versus classical polynomial time computation, and currently we have only noisy gates in the lab, further complicating the goal of achieving quantum supremacy. Given $n$ qubits, circuit depth $d$, and an error rate $\epsilon$, intuitively, we demand the condition,
\begin{align}
...
\begin{align}
q(x) = \vec x^T A \vec x \, (\mathrm{mod} \,4).
\end{align}
...
\begin{mdframed}[innertopmargin=3pt, innerbottommargin=3pt, nobreak]
\texttt{
function ShallowQuantumCircuit():
\begin{enumerate}
    \item Prepare the $n$-qubit state,
    \begin{align}
    \ket{\psi_\mathrm{in}} = \ket{0}^{\otimes n}.
	...
	\begin{align}
		\hat{U}_q \ket{x} = i^{q(x)} \ket{x},
	\end{align}
	for bit-string \mbox{${\vec x}\in \{0,1\}^{n}$}, composed of a product of $CZ$ gates\index{Controlled-phase gates} and phase-shifts\index{Phase-shift gates},
	\begin{align}
...
\end{align}
    ...
    \item The output state is,
    \begin{align}
    \ket{\psi_\mathrm{out}} = \hat{H}^{\otimes n}\hat{U}_q\hat{H}^{\otimes n}\ket{0}^{\otimes n}.
    \end{align}
    \item Measure all qubits in the computational basis, yielding measurement outcomes \mbox{${\vec z}\in \{0,1\}^{n}$}, with probabilities,
    \begin{align}
   	 P(\vec z) = |\bra{\vec z}\hat{H}^{\otimes n}\hat{U}_q\hat{H}^{\otimes n}\ket{0}^{\otimes n}|^2.	
    \end{align}
...
Here a quantum circuit $U_q$ acts on the computational basis states \mbox{$\vec x \in \{ 0,1\}^n$} via a generalised controlled-phase operation,
\begin{align}
U_q \ket{x} = i^{q(x)} \ket{x},
\end{align}
for some oracle function $q(x)$. The solution is obtained by measuring the state $\ket{\Psi_q}$ in the computational basis. Here,
\begin{align}
\ket{\Psi_q} &= H^{\otimes n }U_q H^{\otimes n} \ket{0}^{\otimes n} \nonumber \\
&= \frac{1}{2^n}\sum_{\vec x,\vec z \in \{ 0,1\}^n} i^{q(x)} (-1)^{\vec z^T x}\ket{z}.
\end{align}

The circuit $U_q$ can be decomposed into a product of CZ gates and phase-shift gates, which all commute, thereby allowing the shallow circuit-depth.

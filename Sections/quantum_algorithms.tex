...
If we consider the case of solving a satisfiability problem, to find a satisfying input such that the function evaluates to \mbox{$f(x)=1$}, then it would na{\"i}svely appear that the required quantum operation needs to implement the transformation,
\begin{align}
	\hat{U}_f\ket{x}=\ket{f(x)}.
\end{align}
However, it is easy to see that in general such a transformation is non-unitary, and therefore cannot be implemented quantum mechanically. To overcome this obstacle and enable unitary implementation, we introduce additional ancillary bits\index{Ancillary bits} to $f(x)$ and first write it as a reversible classical circuit\index{Reversible classical circuits}, which then always lends itself to a unitary quantum construction. For example, if we instead introduce some extra qubits and implement the transformation,
\begin{align}
...
\begin{align}
\hat{R}_\theta=\left(\begin{matrix}
1 & 0 \\
0 & e^{i\theta}	
\end{matrix}\right),
\end{align}
...
\begin{itemize}
	...
	\item $n$ controlled-$\hat{U}$s\footnote{Any unitary has a coherently controlled unitary equivalent, of the form $\hat{U}_\mathrm{controlled} = \ket{0}\bra{0}\otimes \hat{\mathbb{I}} + \ket{1}\bra{1}\otimes\hat{U}$. These always have an efficient circuit construction, given $\hat{U}$.}\index{Controlled unitaries}.
...
From the CampBell pairBaker-Hausdorff lemma\index{CampBell pairBaker-Hausdorff lemma} we can approximate this as,
...
\begin{figure}[!htbp]
\includegraphics[clip=true, width=0.4\textwidth]{shallow_circuits_2D}

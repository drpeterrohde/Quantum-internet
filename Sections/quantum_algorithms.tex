...
\subsection{Quantum advantage with shallow circuits}\index{Shallow circuits}
...
It is incredibly difficult to compare quantum VS classical polynomial time computation, and 
currently we have noisy gates. Given $n$ qubits, depth $d$, and an error rate $\epsilon$, intuitively, we need the condition
\begin{align}
nd \ll \frac{1}{\epsilon}
\end{align}
\noindent to hold, in order for a quantum algorithm to run successfully. If $d$ is large, then noise dominates and our circuit is classically simulable; on the other hand, if we have a large $n$ and small $d$ (constant depth circuits), we may observe a quantum speed up in the near future. Given these factors, we are motivated to examine shallow circuits. 
Now we will see that there is a distinction between constant depth circuits run using classical VS quantum algorithm.

The result is for the following hidden linear function (HLF) problem \cite{bravyi2018quantum}. Given an 
...
The quantum alvorithm that solves the 2D HLF problem is similar to the Bernstein and Vazirani problem \cite{bernstein1997quantum}. Here a quantum circuit $U_q$ acts on the basis state $\vec x \in \{ 0,1\}^n$
\begin{align}
...
\begin{figure}[!htbp]
%\includegraphics[trim = 0cm 0cm 0cm 0cm, clip, width=1.0\linewidth]{2DHLFcircuit}
\caption{\comment{Insert figure}\label{fig:2DHLF} Quantum circuit to solve the 2D HLF problem. }
\end{figure}
...
\begin{figure}[!htbp]
\includegraphics[trim = 0cm 0cm 0cm 0cm, clip, width=0.8\linewidth]{2DHLF_grid}
\caption{\comment{Vector version please}\label{fig:2DHLFgrid} Implementation of the HLF problem on a 2D grid.}
\end{figure}

Now, comparing the quantum algorithm to a classical one, it has been proven that any probabilistic classical circuit with bounded input that solves the 2D HLF problem with high probability $\epsilon \leq 1/8$ must have depth increasing logarithmically with input size \cite{bravyi2018quantum}. The speed-up here is provided by quantum non-locality. 

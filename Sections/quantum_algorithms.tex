...
Given noisy circuit, n qubits, depth d, $\epsilon$ error rate. We want this condition
\begin{align}
nd<<1/\epsilon
\end{align}
\noindent for the circuit to be simulable

Deep circuits: few qubits -> classically simulable.

Shallow circuits -> many qubits -> potential for quantum advantage.

Polynomial time computation - the comparison is too difficult. Simpler question - constant depth classical VS constant depth quantum. This would separate BPP from BQP has strong complexity implications. What we can prove: constant depth circuit
...
What sort of quantum problem? Turns out this is a search, or relation problem. A circuit solves a relation problem
if for any input $x$ it outputs a valid solution $z$ (with high probability).
\begin{align}
\sum_{z \in S_X} P(z|x)\geq 1-\epsilon
\end{align}

Result: a problem can be solved with constant depth quantum circuits, but not classical .

The problem is solved with certainty by a constant-depth quantum circuit. Any probabilistic classical circuit with bounded input that solves the problem with high probability $\epsilon \leq 1/8$ must have depth increasing logarithmically with input size.
...
\end{align}
\noindent find a binary vector $\vec z \in {0,1}^n$ such that
...
\end{align}
\noindent where $H$ is the hadamard gate. Destructive interference between terms in x results a vanishing amplitude $\braket{z|\Psi_q}=0$ on basis state $\ket{z}$ which are not solutions of the HLF problem.
The circuit $U_q$ can be decomposed into a product of CZ gates and phase-shift gates $S_i$

\begin{figure}[h]
\includegraphics[trim = 0cm 0cm 0cm 0cm, clip, width=0.45\linewidth]{2DHLFcircuit.pdf}
\caption{\label{fig:2DHLF} }
\end{figure}

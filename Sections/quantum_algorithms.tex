...
\begin{enumerate}
	...
	\end{align}
	for bit-string \mbox{$x\in \{0,1\}^{n}$}, where the oracle is composed of a product of $CZ$ gates\index{Controlled-phase gates} and phase-shifts\index{Phase-shift gates},
	\begin{align}
S_i = \begin{pmatrix}
    1 & 0\\
    0 & -i
...
\end{align}
    ...
    \hat{H}^{\otimes n}.
    \end{align}
    \item Measure all qubits in the computational basis, yielding measurement outcomes ${\vec z}$.
	\item Repeat protocol $O(\mathrm{poly}(n))$ times.
	\item $\Box$
\end{enumerate}
\begin{align}

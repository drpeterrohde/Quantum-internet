...
\subsection{Quantum advantage with shallow circuits}\index{Shallow circuits}\label{sec:shallow_circs}
...
It's incredibly difficult to compare quantum versus classical polynomial time computation, and currently we have noisy gates in the lab. Given $n$ qubits, circuit depth $d$, and an error rate $\epsilon$, intuitively, we demand the condition,
\begin{align}
...
The result is for the following hidden linear function (HLF) problem\index{Hidden linear function problem} \cite{bib:bravyi2018quantum}. Given an  \mbox{$n\times n$} symmetric binary matrix $A$, we specify a quadratic form,
\begin{align}
...
The quantum algorithm that solves the 2D HLF problem is similar to the Bernstein-Vazirani problem \cite{bib:bernstein1997quantum}. Here a quantum circuit $U_q$ acts on the basis state $\vec x \in \{ 0,1\}^n$,
\begin{align}
U_q \ket{x} = i^{q(x)} \ket{x}.
\end{align}
The solution is obtained by measuring the state $\ket{\Psi_q}$ in the computational basis. Here,
\begin{align}
\ket{\Psi_q} &= H^{\otimes n }U_q H^{\otimes n} \ket{0}^{\otimes n} \nonumber \\
&= \frac{1}{\sqrt{2^n}}\sum_{\vec x,\vec z \in \{ 0,1\}^n} i^{q(x)} (-1)^{\vec z^T x}\ket{z},
\end{align}
where $H$ is the Hadamard gate. A schematic of the circuit is given in Fig.~\ref{fig:2DHLF}. Destructive interference between terms in $x$ results a vanishing amplitude \mbox{$\braket{z|\Psi_q}=0$} on basis state $\ket{z}$, which are not solutions of the HLF problem. The circuit $U_q$ can be decomposed into a product of CZ gates and phase-shift gates,
\begin{align}
S_i = \begin{pmatrix}
    1 & 0\\
    0 & -i
...
If we lay out the circuit in 2D (hence the 2D HLF problem), it only requires classically-controlled Clifford gates\index{Clifford gates} between nearest neighbour qubits on a 2D grid. An example is shown in Fig.~\ref{fig:2DHLFgrid}.
\begin{figure}[!htbp]

...
A third experiment performed QKD in a satellite-to-ground\index{Satellite-to-ground communication} configuration, where the ground station was at Xinglong \cite{bib:liao2017satellite}. The protocol that was used was the decoy-state BB84\index{Decoy states}\index{BB84 protocol} protocol, which is robust against a photon-number splitting attack\index{Photon-number-splitting attacks}. A key-rate of between $1-12$kbit/s was achieved while the satellite was visible, depending upon the relative location in the sky, shown in Fig.~\ref{fig:space_4}(d).

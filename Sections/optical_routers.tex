...
\begin{table*}[!htb]
	\begin{tabular}{|c|c|c|}
    	...
    	Single-channel multi-port switch (linear) & \mbox{$N_\text{s}=2n-3$} & \mbox{$2\leq d\leq 2n-3$} \\
  		...
  		Crossbar switch & \mbox{$N_\text{s}=n^2$} & \mbox{$1\leq d\leq 2n-1$}\\
    	\hline
	\end{tabular}
...
A general multi-port switching architecture, that gained popularity in the early days of channel-switched telecommunications networks\index{Channel-switched network}, is the crossbar architecture, whereby $n$ inputs are mapped to $n$ outputs via a binary permutation matrix, which controls a lattice of \mbox{$2\times 2$} switches. The general layout of the architecture is shown in Fig.~\ref{fig:crossbar_switch}, and an example of a routing sequence corresponding to a particular binary control matrix is shown in Fig.~\ref{fig:crossbar_example}. Clearly the scheme requires $n^2$ two-port switches to implement arbitrary \mbox{$n\times n$} mode permutations. The main disadvantage of this scheme is that different paths within a given permutation experience differing optical depth, ranging from 1 to \mbox{$2n-1$}.

\begin{figure}[!htb]
\includegraphics[width=0.3\textwidth]{crossbar_switch}
\caption{Crossbar architecture for multi-port switching. Each orange box represents a \mbox{$2\times 2$} switch, of any physical implementation.} \label{fig:crossbar_switch}	
\end{figure}

\begin{figure}[!htb]
\includegraphics[width=0.3\textwidth]{crossbar_example}
\caption{Example switching configuration for a \mbox{$5\times 5$} crossbar switch. The switch colours represent whether the respective \mbox{$2\times 2$} switch is set to flip (red) the modes or not (orange).} \label{fig:crossbar_example}	
\end{figure}
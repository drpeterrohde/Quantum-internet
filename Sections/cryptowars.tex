...
\begin{enumerate}
\item $\mathrm{cipertext} = \mathrm{plaintext}^e \, (\mathrm{mod} \, n$)
\item return ciphertext
\item $\Box$
\end{enumerate}
function RSADecrypt(ciphertext, privateKey):
\begin{enumerate}
\item $\mathrm{plaintext} = \mathrm{ciphertext}^d \, (\mathrm{mod} \, n)$
...
\begin{align}
	\hat{Z}_i \frac{1}{\sqrt{2}}(\ket{0}^{\otimes n} + \ket{1}^{\otimes n}) \to \frac{1}{\sqrt{2}}(\ket{0}^{\otimes n} - \ket{1}^{\otimes n}),
\end{align}
for any qubit $i$. This invariance in the location of the $\hat{Z}$ gate is the basis for the anonymity of the protocol. If a user wishes to broadcast `0' he does nothing, whereas if we wishes to broadcast `1' he applies the $\hat{Z}$ gate to his local qubit.

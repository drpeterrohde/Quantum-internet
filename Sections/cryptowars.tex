...
RSA \cite{bib:RSA} was the first published public-key cryptographic protocol. It achieves its security based on the belief that factorising large integers into constituent primes is a computationally hard problem. In this scheme, the private key comprises two prime numbers, which are the factors of a larger number, which acts as the public key. It is clear that to obtain the private key from the public key would require solving the integer factorisation problem.

\comment{Trapdoor function}

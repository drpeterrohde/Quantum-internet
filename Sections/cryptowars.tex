...
Alg.~\ref{alg:quantum_voting} describes a quantum implementation for anonymous surveys, using the protocol described by \cite{bib:VaccaroVoting}.
...
\begin{align}
	\ket{B_m} = \frac{1}{\sqrt{N+1}}\sum_{n=0}^m e^{in\Delta_m} \ket{N-n}_T\ket{n}_V,
\end{align}
where,
\begin{align}
\Delta_m = \frac{2\pi}{N+1}\sum_{j=1}^m \nu_j,
\end{align}
...
\begin{align}
\bra{B_N}\hat{T}\ket{B_N} = \sum_{j=1}^N \nu_j,
\end{align}

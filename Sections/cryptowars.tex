...
Because RSA-encrypted messages are long, digital signature protocols do not sign the full document directly. Instead they create a message digest\index{Message digest} of the document using a cryptographic hash function, which is signed using RSA. These hash functions have the property that they cannot for forged or manipulated, providing an accurate summary of a document, but with extremely low memory overhead (256 bits is typical). Hash functions are discussed next in Sec.~\ref{sec:hashing}.
...
\subsection{Bitcoin \& the Blockchain}\index{Blockchain}\index{Bitcoin}\label{sec:bitcoin_blockchain}
...
\subsubsection{Quantum key distribution} \label{sec:QKD} \index{Quantum key distribution}
...
\paragraph{Continuous-variable protocols}\index{Continuous-variable QKD}
...
function CVQKD():
\begin{enumerate}
\item Alice two Gaussian-distributed random numbers with mean zero,
\begin{align}
x_A &= \mathcal{N}(0,V_\text{mod}),\nonumber \\
p_A &= \mathcal{N}(0,V_\text{mod}),
\end{align}
where $V_\text{mod}$ is the modulation variance\index{Modulation variance}.

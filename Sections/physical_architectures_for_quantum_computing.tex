...
We then define a strategy, $f_\text{strat}$\index{Fusion strategies}, for choosing clusters we have stored and bonding them together to form larger clusters. The strategy acts on our cluster vector and updates it accordingly,
\begin{align}\index{Fusion strategy update rules}
\vec{n}_{t+1} = f_\text{strat}(\vec{n}_t).
\end{align}
...
\subsubsection{Bell-pairs}\index{Bell-pairs}\label{sec:CV_bell_pairs}
...
Using a 50:50 beamsplitter to implement the reverse of Bell-pair preparation (described previously in Sec.~\ref{sec:CV_bell_pairs}), one can construct a Bell analyser for performing projections in the Bell basis\index{Bell projections}.

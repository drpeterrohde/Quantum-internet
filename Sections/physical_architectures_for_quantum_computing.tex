...
\comment{To do! Talk about different gate types. Show update table. Reference from fusion strategies section.}
...
\begin{itemize}
	...
	\item Type-II: destroy two photons, but only require on/off detectors, since the gate succeeds upon coincidence events only.
\end{itemize}
...
\begin{figure}[!htb]
\includegraphics[width=0.8\columnwidth]{fusion_gates}
\caption{Linear optics cluster state fusion gates for polarisation-encoded photons. Both type-I and type-II gates employ a single polarising beamsplitter to mediate the entangling measurement. The black bars represent Hadamard gates in the polarisation basis. The type-I gate only measures a single photon, the other freely exiting the gate, which forms a part of the final cluster state. The type-II gate consumes two photons. Because the type-I gate does not measure in coincidence, as per the type-II gate, it requires number-resolved photo-detection, whereas bucket detectors suffice for the type-II gate.} \label{fig:fusion_gates}	
\end{figure}
...
Despite their non-determinism, numerous authors have examined approaches for efficiently preparing arbitrarily large cluster states using these destructive, non-deterministic gates \cite{nielsen, kieling, rohdebarrett, kokBuffer, ???}. 

\comment{These principles can be extended beyond LO to other schemes where entangling gates are inherently non-deterministic or sometimes fail in a heralded manner. E.g hybrid architectures (Sec.~\ref{sec:hybrid}), where a beamsplitter mediates entanglement via which-path erasure, but only sucessfully projects onto an entangled state with probability 1/2.}

\comment{Use micro-clusters as primitives}
...
\comment{Enhance intro}

We will now briefly sketch out the formalism introduced by \cite{bib:RohdeBarrett07} for performing a theoretical analysis of such cluster state preparation schemes. This formalism was initially only presented in the context of linear clusters. However, these act as a primitive resource from which less trivial clusters may be constructed, such as rectangular lattices, which are universal for MBQC.

Consider a non-deterministic KLM-type CZ gate\footnote{In reality, no one would use KLM-type gates for preparing cluster states, owing to their complexity compared to fusion gates. Rather, we use this gate for illustrative purposes, since its operation upon success and failure are very simple for exposition.}, which upon failure destroys its two input photons by measuring them in the computational basis, and leaves the number of qubits unchanged upon success and bonds them together.

To analyze the operation of such a non-deterministic protocol, we begin by defining a vector $\vec{n}_t$ at time $t$, which stores purely classical information. Specifically, the vector tells us how many clusters of every length we have stored in memory (except single qubit clusters, which we assume `come for free'). For example, the second element of the vector tell us how many 3-qubit linear clusters we have in our possession.

We then define a strategy, $f_s$\index{Fusion strategies}, for choosing clusters we have stored and bonding them together to form larger clusters. Note that a key result from this study was that different strategies are hugely different in their ability to prepare long clusters. The strategy acts on our cluster vector and updates it accordingly,
\begin{align}\index{Fusion strategy update rules}
\vec{n}_{t+1} = f_s(\vec{n}_t).
\end{align}
That is, the length vector can be thought of as a series of `buckets' containing clusters of different lengths, and the strategy simply shifts the contents of the buckets around between buckets each time it is applied.

The strategy description, $f_s$, is also responsible for taking care of updating the elements of $\vec{n}$ according to an update rule, which dictates how many photons are lost or gained upon success or failure of the non-deterministic gate. For example, the CZ gate we have employed here for our toy model destroys two qubits upon failure, but upon success creates a cluster of length given by the sum of the lengths of the clusters acted upon by the gate.

We are then interested in the `rate'\index{Cluster state preparation rate}, $r$, at which large clusters are output from the protocol. This is simply extracted by defining a single parameter which counts the number of clusters in $\vec{n}$ above some predetermined target length, and normalises it by the total time taken to reach that point. The rate parameter converges asymptotically for long runtimes. Formally, the rate of preparation is given by,
\begin{align}
r = \lim_{t\to\infty} \frac{N_t}{t},
\end{align}
where $N_t$ is the total number of clusters of length greater than the target length at time $t$. The preparation rate is bounded by \mbox{$0\leq r\leq1$}.
...
One of the key outcomes of this research was that a {\sc Balanced} strategy\index{Balanced strategy} is optimal in terms of preparation rate. This is simply a strategy which preferentially always bonds clusters of equal length.

\comment{These form a resource for more elaborate topology constructions}
...
\comment{Error teleportation upon measuring out leftover qubits. Cite Rohde et al.}

\begin{figure}[!htb]
\includegraphics[width=0.8\columnwidth]{microcluster_linear}
\caption{\comment{To do}} \label{fig:linear_graph}\index{Linear micro-cluster state}
\end{figure}

\begin{figure}[!htb]
\includegraphics[width=0.8\columnwidth]{microcluster_plus}
\caption{\comment{To do}} \label{fig:plus_graph}\index{+ micro-cluster state}
\end{figure}

\begin{figure}[!htb]
\includegraphics[width=0.8\columnwidth]{microcluster_star}
\caption{\comment{To do}} \label{fig:star_graph}\index{Star micro-cluster state}
\end{figure}

\begin{figure}[!htb]
\includegraphics[width=0.8\columnwidth]{microcluster_snowflake}
\caption{Snowflake micro-cluster comprising a regular tree sturcture, with branching parameter \mbox{$b=3$} and depth \mbox{$d=4$}. Multiple copies of this micro-cluster can be placed side-by-side and fused together via attempting to bond the most outward available qubits from neighbouring clusters. The tree structure allows excess qubits to be `pruned' via their truncs rather than leaves, bypassing the need to measure out every single leftover qubit, as is the case, for example, for $+$-clusters. This reduces the number of required pruning measurements from linear to logarithmic, similarly reducing the accumulation of errors associated with pruned qubits.} \label{fig:snowflake_graph}\index{Snowflake micro-cluster state}
\end{figure}

\comment{Figure: pruning trees}

\comment{Honeycomb lattice}
...
\comment{To do! Talk about efficiency}
...
Beyond photonic {\sc BosonSampling}, much investigation has explored the computational hardness of other types of linear optics sampling problems, using states beyond just single photons.
...
While such negative results might be somewhat depressing, it is extremely insightful to understand these regimes, in the interest of avoiding investing excrutiating effort into trying to instead fruitlessly prove that they are hard.
...
Alg.~\ref{alg:positive_P_sim} describes an efficient classical algorithm for simulating the output photo-statistics of a linear optics sampler fed with strictly non-negative $P$-function input states.
...
\begin{enumerate}
	...
	\item Randomly choose a sample $\alpha_m$ from probability density function $P_m(\alpha)$\index{Probablity density function}.
	\setlength{\itemindent}{0in}
		\item \}
		\item Evolve the set of input coherent states through the linear optics network,
		\begin{align}
		\vec\beta = U\cdot\vec\alpha.
		\end{align}
	\item for(m$\in$modes) \{
	...
	\begin{align}
	D_{m,n} = e^{-\frac{|\beta_m|^2}{2}} \frac{{\beta_m}^n}{\sqrt{n!}}.
	\end{align}
...
\comment{Cite Saleh/Ralph etc on positivity of P-function}
...
%
% Quantum Error Correction of Passive Unitary Errors
%

\subsubsection{Quantum error correction of passive unitary errors}\index{Quantum error correction (QEC)}\index{Error averaging}\label{sec:error_averaging}

\comment{Marshman error averaging}

\comment{Redundant copies could in principle be distributed}

\comment{Move to new QEC section}

%
% Continuous Variables
%

\subsection{Continuous variables (CV)} \label{sec:CV_QC} \index{Continuous variable quantum computation}
...
\subsection{Adiabatic quantum computing \& quantum annealing}\index{Adiabatic quantum computing}\index{Quantum annealing}

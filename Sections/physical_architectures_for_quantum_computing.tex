...
The key feature of all these fusion strategies is to employ `micro-clusters'\index{Micro-clusters} as a primitive resource, which enable multiple bonding attempts between them via redundant vertices. We will now outline several of these schemes.
...
\comment{Show Markov chain process diagram. Show algorithm in box.}
...
The central strategy is similar as for linear clusters -- we construct recyclable micro-clusters, which enable multiple bonding attempts. The key difference now is that these redundant vertices must emanate in multiple directions so as to allow the more complex 2D topology.
...
\begin{figure*}[!htb]
\includegraphics[width=0.8\textwidth]{micro_clusters}
...
\comment{Add fig for classical vs quantum discrete-time walk}

...
The worst-case number of edges that must be traversed to reach any vertex from any other is \mbox{$2d=O(\log|V|)$}, known as the \textit{diameter} of the graph\index{Diameter}, which implies that accumulated cost metrics scale as \mbox{$c=O(\log |V|)$}. Trees are the most frugal graphs in their number of edges, which are fixed at \mbox{$|E|=|V|-1$}, irrespective of the branching parameters, since because the graph is strictly acyclic, every addition of an edge requires the addition of exactly a single vertex. This makes tree graphs the cheapest to construct in terms of physical resource usage.

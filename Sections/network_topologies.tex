...
A summary of the mathematical characteristics of the topologies presented is shown in Table.~\ref{tab:net_top_sum}.\comment{Finish this text - expand on why these properties matter}
...
\begin{table*}[!htbp]
	\begin{tabular}{|c|c|c|c|}
		\hline
      	...
      	Linear & $|V|$ & \mbox{$O(|V|)$} & \mbox{$O(|V|)$} \\
      	Complete & $|V|$ & \mbox{$O(|V|^2)$} & $1$ \\
      	Lattice & $mn$ & \mbox{$O(mn)$} & \mbox{$O(m+n)$} \\
      	Tree & $|V|$ & \mbox{$O(|V|)$} & \mbox{$O(\log |V|)$} \\
      	Percolation & \mbox{$O(p_\mathrm{vertex}\cdot |V|)$} & \mbox{$O(p_\mathrm{edge}\cdot |E|)$} & variable \\
      	Random & \mbox{$O(p_\mathrm{vertex}\cdot |V|)$} & \mbox{$O(p_\mathrm{edge}\cdot |V|^2)$} & variable \\
...
\comment{Show average edge and vertex numbers}

...
According to the Bianconi-Barab{\'a}si fitness model\index{Bianconi-Barab{\'a}si fitness model} \cite{bib:BBfitness, bib:BAfitness}, let \mbox{$\eta_i>0$} be the \textit{fitness factor}\index{Fitness factors} of node $i$, which follow a distribution $\rho(\eta)$, characteristic of the network. Then the fitness parameters\index{Fitness parameters} are defined to be normalised such that,
\begin{align}

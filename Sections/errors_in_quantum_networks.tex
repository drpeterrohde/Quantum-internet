...
\begin{figure}[!htb]
	...
	\caption{Model for the loss channel. The input state, $\hat\rho_\mathrm{in}$, passes through a beamsplitter of transmissivity $\eta$, and the reflected mode discarded, yielding the lossy output state \mbox{$\hat\rho_\mathrm{out} = \mathcal{E}^\mathrm{loss}_\eta(\hat\rho_\mathrm{in})$}.} \label{fig:loss_model} \index{Loss channel}
\end{figure}

...
\subsection{QKD versus classical key exchange techniques}\index{Classical key exchange}

A 2016 assessment from the UK agency GCHQ\index{GCHQ} references four concerns with QKD as an effective replacement for classical techniques. These concerns are:
\begin{itemize}
...
\subsection{Using and re-charging QuSTICK quantum entanglement}\index{QuSTICK!Re-charing}\index{QuSTICK!Utilisation}
...
\subsection{Revisiting the criticisms from GCHQ}\index{GCHQ}
...
\textbf{Denial of service (DoS)\index{Denial of service (DoS) attacks} attacks that interfere with the paths carrying the QKD transmissions also seem potentially easier with QKD than with contemporary Internet or mobile network technologies. Since QKD devices typically abort a key establishment session when they detect tampering, this makes it difficult to recommend QKD for contexts where DoS attacks are likely to be attempted.}

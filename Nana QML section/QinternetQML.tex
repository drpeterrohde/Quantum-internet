...
Today, classical machine learning\index{Classical machine learning} is affecting the understanding and better regulation of the classical networking infrastructure underpinning the modern internet. This includes pattern recognition for individual computing devices, security and fault management, routing and traffic management, resource management, and distributed computation. In these applications, it is not only the speed of processing that is important, but reliability and security can also be paramount.
...
\section{Classical machine learning in classical networks}\index{Classical machine learning!Classical networks}
...
Machine learning algorithms allow us to make predictions about a current or future dataset without requiring explicit instructions. Since its aims are directed more towards \textit{prediction}\index{Prediction} rather than purely \textit{estimation}\index{Estimation}, it differs from the field of statistical estimation, although it shares many techniques in common.

There are three mains paradigms for machine learning:
\begin{itemize}
	\item Supervised learning\index{Supervised learning}: relies upon training data from which inferences and predictions about new test data can be extracted.
	\item Unsupervised learning\index{Unsupervised learning}: makes inferences from the data at hand without access to a training stage, much like a student learning without an instructor.
	\item Reinforcement learning\index{Reinforcement learning}: operates using a different framework and aims to find the best action steps in a particular environment to maximise a given reward.
\end{itemize}
...
\subsection{Security \& fault management}\index{Security}\index{Fault management}
...
\subsubsection{Anomaly detection \& fault management}\index{Anomaly detection}\index{Fault management}
...
\subsubsection{Adversarial machine learning}\index{Adversarial machine learning}
...
\subsection{Traffic management \& routing }\index{Traffic management}\index{Routing}

The effective operation of a large-scale network also requires automated management. This includes efficient means for traffic prediction, traffic classification, routing, and congestion control\index{Congestion control}. Machine learning algorithms have been developed for all of these.

\subsubsection{Traffic}\index{Traffic management}

Predicting network traffic is becoming increasingly important, especially in diverse and complex networks. This is commonly addressed using time-series forecasting\index{Time-series forecasting} (TSF) methods. This can make use of either statistical analysis models or supervised machine learning methods \cite{bib:bermolen2009support, bib:chabaa2010identification, bib:cortez2006internet}. Non-TSF methods also exist \cite{bib:chen2016predicting, bib:li2016inter}.

The most commonly-used technique for traffic classification is the so-called flow feature-based technique\index{Flow feature}. This takes into account information on unidirectional packets sent across the network. For this, supervised machine learning has been found to be accurate for traffic classification. However, unsupervised techniques have been found to be more robust. Their joint application is a very powerful technique \cite{bib:erman2007offline, bib:zhang2015robust}.

\subsubsection{Routing}\index{Routing}

Machine learning is most applicable to dynamic routing problems, requiring rapid updating of optimal routes. Otherwise, existing routing tables can be sufficient. Since these settings depend on frequent recalculation, reinforcement learning algorithms are the most appropriate. In particular, Q-learning\index{Q-learning} has performed well in various networks \cite{bib:wang2006adaptive, bib:forster2007froms, arroyo2007q}.

\subsubsection{Congestion control}\index{Congestion control}
...
\subsection{Distributed machine learning}\index{Distributed machine learning}
...
\subsubsection{Hybrid algorithms}\index{Hybrid algorithms}
...
In QAOA, the ground state reached then encodes the classical solution to a classical optimisation problem, like \textsc{MaxCut}\index{MaxCut}, with polynomial runtime. Thus, it is not a quantum-enhanced algorithm for a classical machine learning problem, but rather exploits a classical machine learning algorithm. It remains to be seen if optimisation problems more directly relevant to networking applications can be solved in this way.
...
\subsection{Security \& other applications}\index{Security}

\subsubsection{Anomaly detection}\index{Anomaly detection}
...
\subsubsection{Adversarial quantum machine learning}\index{Adversarial machine learning!Quantum}
...
\subsection{Distributed quantum machine learning}\index{Distributed machine learning!Quantum}
...
\subsubsection{Tomography}\index{Tomography}
...
\subsubsection{Separability}\index{Separability}
...
\subsubsection{Automated experiment design}\index{Automated experiment design}
...
\subsubsection{Variational quantum eigensolvers}\index{Variational quantum eigensolvers}
...
\subsubsection{Template matching}\index{Template matching}

...
Clearly the goal when choosing routing strategies is to minimise the net cost, Eq.~\ref{eq:net_cost_R}. That is, solving the optimisation problem,
\begin{align}
...
\peter{When considering strategy optimisation, the first question to ask is `Who performs the optimisation, and who has access to what information?'.
...
In the case of {\sc Individual} algorithms, the required knowledge of the state of the network could be obtained using network exploration algorithms (see Sec.~\ref{sec:path_exp}). On the other hand, for {\sc Central} algorithms, either network exploration could be employed, or alternately the network policy could require nodes to notify the central authority upon joining or leaving the network. The former introduces an overhead in classical networking resource usage, since network exploration must be performed routinely to keep the central ledger up to date. The latter, on the other hand, avoids this, but introduces a point of failure, in that all network participants must be reliable in notifying the central authority as required by the network policy.}

\subsubsection{Local vs. Global Optimisation}

\peter{There are two general approaches one might consider when choosing strategies -- \emph{local optimisation} ({\sc Local}) and \emph{global optimisation} ({\sc Global}). {\sc Local} simply takes each state to be communicated, one-by-one, and allows it to individually choose an optimal routing strategy based on the state of the network at that moment. {\sc Global} is far more sophisticated and simultaneously optimises the sum of the routing costs, Eq.~\ref{eq:c_total}, of all currently in-demand routes.
...
If one were to become even more sophisticated, one might even envisage treating network resource allocation in a game theoretic context, which we won't even begin to go into here.
...
\peter{The notion of dephasing can be easily generalised to non-qubit systems. In general, dephasing has the property of mapping a superposition of basis states to a mixture of the same basis states, whilst preserving amplitudes. Thus, for perfect dephasing,
\begin{align}
\mathcal{E}^\mathrm{dephasing}\left(\sum_i \alpha_i\ket{i} \cdot \sum_j \alpha_j^*\bra{j} \right) \to \sum_i |\alpha_i|^2 \ket{i}\bra{i},
\end{align}
for some arbitrary basis enumerated by $i$ and $j$. For partial dephasing, we can express the channel as creating as a mixture over the input state with different phase rotations applied,
\begin{align}
\mathcal{E}(\hat\rho) = \int_{0}^{2\pi} \phi(\omega) \hat{\Phi}(\omega)\hat\rho\,\hat{\Phi}(\omega)^\dag\,d\omega,
\end{align}
where $\hat{\Phi}(\omega)$ is a phase-shift operator with phase $\omega$, and $\phi(\omega)$ characterises the probability distribution of phase-shifts. In the case of optical states, the phase-shift operators take the form,
\begin{align}
\hat\Phi(\omega) = e^{i\omega\hat{n}},
\end{align}
in the photon-number basis, where $\hat{n}=\hat{a}^\dag\hat{a}$ is the photon-number operator.}
...
\end{align}
where \mbox{$0\leq\eta\leq 1$} quantifies the degree of damping. The physical intuition is clear upon inspection of the structure of the projectors in the Kraus operators.

The degree of amplitude damping is often quoted in terms of a channel's $T_1$ time, characterising the expected time for a $\ket{1}$ state to undergo spontaneous emission and relax to the $\ket{0}$ state.
...
\end{align}
since the eigenvalue spectrum of a density operator is invariant under unitary transformation. Therefore, the mutual information represents the maximum amount of information Bob can learn about Alice's state under \emph{any} local operations.
...
The motivation for the dual network is to ensure that classical and quantum data that jointly represent packets (see Sec.~\ref{sec:prot_stack}) remain synchronised and subject to the same QoS issues, such as packet collisions (see Sec.~\ref{sec:collision}) and network congestion.
...
\section{Interconnecting \& Interfacing Networks} \label{sec:inter}
...
\item Physical interface: interconnect may be switching between different media. Such physical interfaces have costs associated with them. For example, coupling between free-space and fibre is typically very lossy. This is discussed in Sec.~\ref{sec:opt_int}.
...
\peter{Both BFS and DFS guarantee visiting every vertex in a connected graph, and do so using only nearest neighbour transitions. Such algorithms are therefore very useful for network discovery.
...
\peter{To implement the shortest-path algorithms discussed above, the party performing the calculation requires knowledge of the full network graph. In an ad hoc network, where users might be added to or removed from the network arbitrarily, this isn't necessarily the case.
...
Of course, single photons are just the textbook example of optical quantum state engineering. Countless other far more elaborate states are very difficult to prepare, and it would be highly desirable if they could be obtained over a quantum network.
...
\end{align}
may be exploited to perform Heisenberg-limited quantum metrology, allowing extremely precise phase measurement with large photon-number $N$. However, these states are notoriously difficult and technologically challenging to prepare, and can only prepared non-deterministically using linear optics.
...
\begin{itemize}
    ...
    \item Bell state projection, when using polarisation-encoded photons, can be implemented deterministically using a CNOT gate (e.g a KLM gate), or non-deterministically using a PBS (with success probability of $1/2$, since it is only able to distinguish two of the four Bell states). The latter only requires high HOM visibility, avoiding the need for interferometric stability.
\end{itemize}
...
\peter{Unless the entire pipeline of quantum operations through the course of a protocol is all-optical, there will be a need to exchange information between physical systems, i.e light-matter interactions. The archetypal interface is that between a photonic qubit in the \mbox{$\{\ket{0},\ket{1}\}$} photon-number basis, and a two-level matter qubit, such as an atom in a cavity, an atomic ensemble, a nitrogen-vacancy (NV) centre, or a quantum dot. In this case, the interface can be expressed as an interaction Hamiltonian of the form,
\begin{align}
\hat{H}_\mathrm{int} = \hbar g (\hat{a}\,\hat\sigma^+ + \hat{a}^\dag\hat\sigma^-),
\end{align}
where $\hat{a}$ ($\hat{a}^\dag$) is the photonic annihilation (creation) operator, $\hat\sigma^\pm$ are the Pauli spin-flip operators, and $g$ is the interaction strength. The interpretation of this Hamiltonian is very clear upon inspection -- the annihilation (creation) of a photon is associated with the excitation (relaxation) of the two-level matter system, thereby directly exchanging quantum information between the two systems, as shown in Fig.~\ref{fig:opt_int}.}

\begin{figure}[!htb]
\includegraphics[width=0.6\columnwidth]{opt_inter}
\caption{\peter{Light-matter interfacing between a single-photon state and a two-level matter qubit. The absorption (emission) of a photon is associated with the excitation (relaxation) of the matter qubit.}} \label{fig:opt_int}
\end{figure}
...
A key observation in the BB84 protocol is that it requires no entanglement, no mode-matching, no interference, and no interferometric stability. It can be realised using nothing more than single-photon sources, waveplates, and photodetectors. For these reasons, QKD is a technology that is entirely viable on a large scale using present-day technology.
...
\subsubsection{Quantum Anonymous Broadcasting}
...
\comment{Discuss NOON states}

\comment{Discuss MORDOR}
...
The attractive feature of this type of approach is that the actual entanglement is generated using all-optical operations, despite the underlying logical qubits being stationary, and potentially physically separated. This allows the entangling operations to be performed remotely in the cloud, without moving the qubits. Such hybrid systems present an interesting platform for cloud quantum computing -- despite the qubits being stationary, we are able to outsource the interactions between them to distant servers.

Expanding upon this idea, we can envisage that future quantum networks with arbitrary underlying physical architectures will require \emph{interfaces}, coupling the quantum states of different types of physical systems for representing qubits. It is most likely that optical interfacing will be the major player here, since it allows speed-of-light interfacing between distant qubits. Optical interfacing is discussed in Sec.~\ref{sec:opt_int}.

\comment{Say much more about this}
...
Let us perform a rudimentary analysis of the resource scaling of this type of approach. The probability of successfully creating an edge between two logical qubits is,
\begin{align}

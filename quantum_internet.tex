...
\item Rather than transmitting quantum states between distant parties directly, if the communications channel fails (e.g via loss), the state to be transmitted is not corrupted. Instead we can wait for the next successfully transmitted Bell-pair until we are ready to teleport the state, which then proceeds without utilising the quantum communications channel and its associated costs.
\end{itemize}

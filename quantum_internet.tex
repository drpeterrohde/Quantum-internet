...
\begin{enumerate}
\item We wish to apply a CNOT gate to the two-qubit state \mbox{$\ket\psi_{A}\ket\phi_{B}$} to obtain \mbox{$\hat{\mathrm{CNOT}}_{A,B} \ket\psi_{A}\ket\phi_{B}$}.
\item We introduce two additional qubits, $C$ and $D$, and use state teleportation to teleport $\ket\psi_\mathrm{A}$ onto $C$, and $\ket\psi_{B}$ onto $D$, preparing $\ket\psi_{C}\ket\phi_{D}$.
...
\item The CNOT now acts jointly upon the EPR pairs that were acting as a resource for the state teleportation, independent of \mbox{$\ket\psi_{A}\ket\phi_{B}$}.
\item Group the CNOT gate and EPR pairs together, and treat them as a 4-qubit resource state preparation stage, which does not depend on \mbox{$\ket\psi_{A}\ket\phi_{B}$}, and can therefore be prepared offline in advance.
\item The 4-qubit resource state is of the form \mbox{$\hat{\mathrm{CNOT}}_{2,3}\ket{\Psi^+}_{1,2}\ket{\Psi^+}_{3,4}$}.
\item If the CNOT is non-deterministic, employ a trivial {\sc Repeat Until Success} approach for resource state preparation.
\item Now implementing $\hat{\mathrm{CNOT}}_{A,B} \ket\psi_{A}\ket\phi_{B}$ is reduced to the problem of: (1) offline preparation of highly-entangled 4-qubit resource states; (2) two Bell measurements (inherited from the state teleportation protocol); and, (3) some configuration of Pauli operators, dependent upon the Bell measurement outcomes.
\item The output state is \mbox{$\hat{\mathrm{CNOT}}_{C,D} \ket\psi_{C}\ket\phi_{D}$}.
\item $\Box$
\end{enumerate}}}
\caption{Teleporting a CNOT gate onto a two-qubit state. The key observation is that at the end of the circuit manipulations, the CNOT is not acting directly on the target qubits, only on Bell-pairs. Thus, the CNOT gate and Bell-pairs can be grouped together and treated as an offline resource state preparation stage, for which non-determinism is easily accommodated via {\sc Repeat Until Success}, unlike direct application of the CNOT to the target qubits, which would corrupt them upon gate failure, potentially ruining an entire computation.} \label{tab:gate_teleport}
\end{table}

...
is the photon-number of the $i$th mode.
   ...
   \item Alice chooses a random private key from the uniform distribution,
   \begin{align}
    k\in(0,2\pi).
    \end{align}
    \item Alice prepares the encoded state by applying the same polarisation rotation, $k$, to each mode,
   \begin{align}
   ...
   \end{align}
    ...
    \item Bob applies processing using his linear optics network, to obtain,
    \begin{align}
    ...
    \item $\Box$
\end{enumerate}}}
\caption{Protocol for implementing homomorphic encryption on photonic, passive linear optics. This includes {\sc BosonSampling} and photonic quantum walks.} \label{tab:homo_LO}
\end{table}
...
\end{align}
for sufficiently large $m$, which asymptotically approaches 0. Furthermore, because a single computation requires Alice to perform only a single call to Bob's algorithm, which we treat as a black box, Alice gains minimum knowledge about Bob's secret algorithm.

...
\item Single-qubit gates are denoted as boxes containing the name of the associated unitary operation (e.g the Hadamard ($\hat{H}$), Pauli bit-flip ($\hat{X}$), Pauli bit-phase-flip ($\hat{Y}$), and Pauli phase-flip ($\hat{Z}$) gates).
...
\end{align}
\caption{The single-qubit teleporter, based upon a controlled-phase (CZ) gate, a single-qubit measurement, and classical feedforward.} \label{fig:single_qubit_teleporter}
\end{figure}
...
\item Time- or frequency-multiplexing: each user is designated a particular frequency spectrum or series of time-slots exclusively for their use. \index{Time- \& frequency-multiplexing}
\item Code Division Multiple Access (CDMA): all users can broadcast over a channel simultaneously, and the construction of the coding technique enables demultiplexing of the distinct signals, despite their interference with one another.\index{Time- \& frequency-multiplexing}
...
\subsubsection{Local vs. global optimisation} \index{Local \& global optimisation}
...
\end{align}
for normalisation. Here $\mathcal{E}$ is a super-operator, denoting the action of the process on state $\hat\rho$. This is also referred to as the \emph{operator-sum representation}. This representation has the elegant interpretation as the probabilistic application of each of the Kraus operators $\hat{K}_i$, with probability,
\begin{align}
...
\begin{align} \label{eq:single_photon_enc}\index{Polarisation qubit encoding} \index{Dual-rail qubit encoding}
...
\begin{itemize}
\item \index{Two-level atoms}Two-level atoms: let two distinct electron energy levels, with long lifetimes, represent the two logical basis states.
\item \index{$\lambda$-configuration atoms}$\lambda$-configuration atoms: atoms with two degenerate ground states, which encode the logical qubit, and an additional excited state, which may be transitioned to upon excitation from only one of the ground states. Relaxation from the excited state enables optical coupling via the emitted photon.
...
\item \index{Nitrogen-vacancy centres}Nitrogen-vacancy (NV) centres: are a type of point defect in diamond, which has a very well defined energy level structure that may be utilised to represent qubits.
...
\subsection{Costs as distance metrics} \label{sec:cost_as_dist} \index{Distance metrics}
...
\section{Collision handling \& classical errors} \label{sec:collision} \index{Collision handling}
...
\subsection{Point-to-point (P2P)} \index{Point-to-point topologies}
...
\subsection{Shortest-path} \label{sec:shortest_path} \index{Dijkstra's shortest-path algorithm}
...
\subsubsection{Single-photons} \label{sec:single_phot_src} \index{Single-photon preparation}
...
\subsubsection{Matter qubits} \index{Matter qubit state preparation}
...
In general, the protocol is deterministic, although using PBSs to perform partial Bell measurements, the success probability is $1/2$.
...
\end{align}
which for \mbox{$N\gg 1$} approaches the pure state $\ket{\psi_W^{(N-1)}}$, i.e a W-state with one fewer particle.
...
\begin{align} \label{eq:NS_trans}\index{Non-linear sign-shift gate}
...
\subsection{The global virtual quantum computer} \label{sec:marketplace} \index{The global virtual quantum computer}
...
However, as discussed in Sec.~\ref{sec:dist_QC}, the computational power of a quantum computer does not scale linearly with its number of qubits, but exponentially. This completely changes the economics, and market dynamics of networked quantum computers. Intuitively, we expect equitable time-sharing of unified quantum computational resources to offer more performance to all users than if they were to exclusively use their own resources in isolation. That is, the cost of a computation is reduced by resource-sharing, even after time-sharing.
...
\item \emph{Displacement-key encoding} (Sec.~\ref{sec:disp_key_enc}): an arbitrary configuration of random phase-space displacements is applied to the input modes, for which no security proofs have yet been performed.
\end{itemize}
...
\subsection{Entanglement swapping \& quantum repeaters} \index{Entanglement swapping} \index{Quantum repeaters}
...
\subsubsection{Nitrogen-vacancy centres} \index{Nitrogen-vacancy centres}
...
\subsubsection{Superconducting rings} \index{Superconducting ring detectors}
...
\subsubsection{Linear optics}

\subsubsection{Boson-sampling \& quantum walks}

\subsubsection{Ion traps}

\subsubsection{Superconducting circuits}

\subsubsection{Nitrogen-vacancy centres}

\subsubsection{Adiabatic quantum computing \& quantum annealing}
...
\subsection{The era of quantum supremacy} \label{sec:era_quant} \index{The era of quantum supremacy}
...
\subsection{The economics of the quantum internet} \label{sec:economics} \index{The economics of the quantum internet}
...
\subsection{The quantum singularity} \label{sec:singularity} \index{The quantum singularity}
...
\section{Conclusion -- The vision of the quantum internet} \label{sec:vision_quant} \index{The vision of the quantum internet}

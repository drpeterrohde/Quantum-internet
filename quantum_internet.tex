...
\end{align}
may be exploited to perform Heisenberg-limited quantum metrology, allowing extremely precise phase measurement with large photon-number $N$ \cite{bib:Dowling08}. 
...
\end{align}
where $\hat{n}$ is the photon-number operator associated with the second mode. Then, a simple interferometric procedure is able to extract this enhanced phase-dependence as an observable.
...
\end{align}
Here $n_x$ denotes the set of vertices neighbouring $x$, and $A^{(x)}$ is a unitary matrix representing the weights of edges within this neighbourhood. The total time evolution is defined analogously to before,
\begin{align}
...
It was shown by \cite{bib:RohdeMultiWalk11} that any such walk can be efficiently represented using a linear optics decomposition comprising at most $O(|V|^2)$ optical modes. 
...
\end{enumerate}}}
\caption{Using which-path erasure to entangle two $\lambda$-configuration matter qubits via post-selected linear optics.} \label{tab:which_path}
\end{table}

...
Let \mbox{$k=\{k_1,\dots,k_m\}$} be a partition of the key $k$ into sub-keys $\{k_i\}$, one associated with each mode $i$. Let $\hat{E}_i(k_i)$ and $\hat{D}_i(\tilde k_i)$ be the encoding and decoding operations for the $i$th mode. $\tilde k$ is a potentially transformed version of $k$, to accommodate that the encryption and decryption keys may be asymmetric. Let $\hat{U}$ be the computation. Then, separability of the encoding and decoding operations requires the following commutation relation to hold,
...
\hat\rho_\mathrm{encoded} = \sum_k \hat{E}(k)\ket\psi_\mathrm{plaintext}\bra\psi_\mathrm{plaintext} \hat{E}^\dag(k),
\end{align}
where,
\begin{align}
\hat{E}(k) = \bigotimes_{i=1}^m\hat{E}_i(k_i).
\end{align}
To minimise Bob's chances of guessing Alice's state, we would like to maximise the von Neuman entropy of Bob's state. For \mbox{$S(\hat\rho_\mathrm{encoded})=0$} we have no secrecy, whereas for maximal \mbox{$S(\hat\rho_\mathrm{encoded})$} we have perfect secrecy (for the given plaintext basis state).
\end{definition} 
...
\cite{siHuiTan} evaluated the security of this protocol in the case where the basis states were restricted to the binary $\ket{\pm\alpha}$ states. Thus, each input mode encodes at most a single bit (approaching one bit for \mbox{$\alpha\to\infty$}). They were unable to calculate information-theoretic bounds (as is often the case), and instead resorted to the alternative approach of calculating the distinguishability of codewords under the trace distance. If the basis codeword states observed by Bob are indistinguishable, he cannot guess Alice's plaintext state, whereas if they are distinguishable, he can.
...
\caption{\comment{To do} Trace distance between encoded and unencoded basis states for coherent state computation using phase-key encoding. Each of the $m$ modes is inputted with one of two basis coherent states, $\ket{\pm\alpha}$. $\mathrm{wt}(\vec{x})$ denotes the Hamming weight of bit-string $\vec{x}$. Two states are indistinguishable if their trace distance is 0, and orthogonal if their trace distance is 1. Lower trace distance between encoded states implies a lower chance of Bob guessing Alice's plaintext input state.} \label{fig:homo_coh_st_tr}
\end{figure}
...
\comment{To do! Mathematical details! How does the secrecy of this scheme compare to the secrecy of a classical homomorphically encrypted matrix multiplication? How many bits are hidden? What mathematical techniques are used for the derivation? Add figures for trace distance with and without encryption. Show scaling against both $\alpha$ and $m$.}
...
\begin{align}
\hat{D}_i(\alpha_i) \to \prod_{j=1}^m \hat{D}_j(\beta_j),
\end{align}

...
\caption{Trace distance between unencoded (top) and encoded (middle) basis states for coherent state computation using phase-key encoding, and the ratio between them, \mbox{$R=D_\mathrm{enc}/D_\mathrm{unenc}$} (bottom), with \mbox{$d=50$} keys and \mbox{$m=10$} modes. Each mode is inputted with one of two basis coherent states, \mbox{$\ket{\pm\alpha}$}. \mbox{$\mathrm{wt}(\vec{x})$} denotes the Hamming weight of bit-string $\vec{x}$. Two states are indistinguishable if their trace distance is 0, and distinguishable if their trace distance is 1. Lower trace distance between encoded states implies a lower chance of Bob guessing Alice's plaintext input state. \mbox{$R<1$} implies information hiding from Bob.} \label{fig:homo_coh_st_tr}
\end{figure}

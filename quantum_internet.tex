...
\caption{The topology of an ethernet network, whereby all users share a common channel, which they can broadcast to at their leisure. If data packets collide, it is detected via the packets' checksums, and the corrupted packets may be re-broadcast after a random `backoff' waiting period, repeating this process until packet delivery is successful.} \label{fig:ethernet}
\end{figure}
...
%
% Optical Quantum Computing Using Weak Cross-Kerr Non-Linearities
%

\subsection{Optical quantum computing using weak cross-Kerr non-linearities}
...
\end{align}
for sufficiently large $m$ and $d$, which asymptotically (but unfortunately only polynomially\footnote{Note that an exponentially small bound is actually prohibited the no-go theorems for oblivious transfer and bit commitment \cite{bib:HKLo97, bib:SpekkensRudolphSecure}}) approaches 0.
...
The other obvious candidate is \emph{displacement key encoding}, whereby random displacements in phase-space (Sec.~\ref{sec:non_lin_opt}) form the encoding operation. Displacement operators exhibit the property that a tensor product of displacement operators commute through linear optics circuits to yield a different combination of tensor products of displacement operators, where the displacement amplitudes obey the relationship from Eq.~\ref{eq:betaUalpha}. Specifically,
\begin{align}
\hat{D}_i(\alpha_i) \to \prod_{i=1}^m \hat{D}_j(\beta_j),
\end{align}
where $\vec\alpha$ and $\vec\beta$ are related according to Eq.~\ref{eq:betaUalpha}.

Based upon this observation, if the unitary $\hat{U}$ were known to Alice (i.e no secrecy for Bob's algorithm), she could efficiently encode and decode her state, since determining $\vec\beta$ from $\vec\alpha$ requires only classically-efficient matrix multiplication.

Although no work has performed any security proofs for such an encoding, this is a candidate approach that warrants future work. Furthermore, it is plausible that this approach might apply to a broad class of optical states, since, unlike phase-rotations, no optical states are invariant under non-zero displacements.

\comment{Does the Fitzsimons group mind me discussing this, given that we haven't fleshed it out yet?}
...
Thus, we see that Bell-pairs form a ubiquitous resource, covering the most significant quantum protocols -- quantum computation, distributed quantum computation, quantum state and gate teleportation, and quantum cryptography. This warrants special treatment of entanglement distribution, as a fundamental building block for the quantum internet.

...
Essentially, the long coherence lifetimes of collective excitations owes to the fact that the excitation is effectively encoded as a W-state, an equal superposition of a single excitation across many modes, of the form,
\begin{align}
...
+&\dots).	
\end{align}
W-states are favourable from a decoherence perspective as tracing out a single mode has minimal impact on the coherence of the residual state, which preserves most entanglement, with this robustness growing with the number of modes. This is in stark contrast to GHZ states, which completely decohere under the loss of just a single mode.
...
\end{align}
Note that the economic leverage is strictly greater than unity, \mbox{$\lambda_i>1\,\forall\, i$}. Thus, for any user, irrespective of the size of their quantum computer, they are better off joining the distributed network and claiming a time-share, than utilising only their own hardware full-time. Similarly, the distributed network is better off accepting them into the network. This is in contrast to classical networks, where \mbox{$\lambda\approx 1$}, for any number of nodes in the network.

...
\comment{Only discuss models relevant to optical interconnection. There are many other models that we will not describe in detail, such as adiabatic quantum computation and quantum simulated annealing.}
...
The \emph{circuit model} is the conventional and intuitive approach for expressing quantum algorithms, decomposing algorithms into sequences of elementary gates from a universal gate set.

\comment{To do} Fig.~\ref{fig:eg_circuit}
\cite{bib:NielsenChuang00}

\begin{figure}[!htb]
\begin{align}
\Qcircuit @C=.7em @R=.4em @! {
\lstick{\ket{\psi_1}} & \qw & \qw & \ctrl{1} & \gate{Y} & \meter & \control \cw\\
\lstick{\ket{\psi_2}} & \qw & \targ & \targ & \qw & \meter & \cwx\\
\lstick{\ket{\psi_3}} & \gate{H} & \ctrl{-1} & \qw & \qw & \gate{X} \cwx & \gate{Z} \cwx & \rstick{\ket{\phi}} \qw
} \nonumber
\end{align}
\caption{Simple example of a quantum circuit, comprising several quantum gates and measurements. Rows represent qubits, and time flows from left to right.} \label{fig:eg_circuit}
\end{figure}
...
\end{align}
\caption{The single-qubit teleporter, based upon a controlled-phase (CZ) gate, a single-qubit measurement, and classical feedforward. \comment{Add to text!}} \label{fig:single_qubit_teleporter}
\end{figure}
...
\comment{To do. We will focus on optical architectures and ones which can be optically interfaced for the purposes of networking.}
...
\comment{To do! Discuss both circuit model and cluster states.}
...
A case of particular interest is the \emph{single qubit leverage} -- the leverage on the investment into adding a single additional qubit to the global network. This is an indicator of the economic return on investment into expanding the network. This is simply given by,
\begin{align}
\lambda_\mathrm{single} = \beta^{n_\mathrm{global}} (1+n_\mathrm{global}),
\end{align}
...
\end{align}
The state observed by Bob is the mixture of Alice's plaintext over the complete set of encoding operations,
...
\end{align}
\comment{What's the limit as $d\to\infty$?} where $\mathcal{E}$ denotes the mixture over encoding operations observed by Bob, as per Eq.~\ref{eq:mix_over_enc_ops}, and there are $d$ distinct keys. These relationships are illustrated in Fig.~\ref{fig:homo_coh_st_tr}. Clearly, the encoded basis states exhibit greater indistinguishability than the unencoded ones, demonstrating that information is hidden from Bob. Unfortunately, the trace distance does not have the elegant interpretation of `number of bits hidden' that the mutual information does. Nonetheless, this demonstrates partial hiding of information.

...
\begin{align}
\left[\bigotimes_{i=1}^m\hat{E}_i(k_i)\right]\ket\psi \neq \ket\psi.
\end{align}
\end{definition} 
...
This encryption technique applies to more than just linear optics, but extends to also include generalised non-linear phase-shift operations, generated by Hamiltonians that are polynomials of the photon-number operator,
\begin{align}
\hat{H} = O(\mathrm{poly}(\hat{n})).
\end{align}
This observation follows trivially from the observation that the phase-shift encoding operations (which are generated by a Hamiltonian linear in the photon-number operator) commute with any polynomial in the photon-number operator,
\begin{align}
[\hat{n},\mathrm{poly}(\hat{n})] = 0.
\end{align}

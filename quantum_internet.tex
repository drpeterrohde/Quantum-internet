...
\begin{itemize}
\item Searching unstructured databases (Grover's algorithm \cite{bib:Grover96} -- quadratic speedup).\index{Quantum algorithms!Grover's algorithm}
\item Satisfiability problems\footnote{A satisfiability problem is one where we search a function's input space for a solution(s) satisfying a given output constraint. The hardest such problems, like the archetypal {\sc 3-SAT} problem, are \textbf{NP}-complete.}\index{Quantum algorithms!\textbf{NP}-complete problems} (Grover's algorithm -- quadratic speedup). This includes solving \textbf{NP}-complete problems, and brute-force cracking of private encryption keys\footnote{Note that when performing a brute-force attack against a private encryption key, a quadratic speedup effectively halves the key length in terms of algorithmic runtime. Thus, in the quantum era private key lengths will need to be doubled.}. 
\item Period finding and integer factorisation\index{Quantum algorithms!Shor's algorithm} (Shor's algorithm \cite{bib:ShorFactor} -- exponential speedup). This compromises Rivest, Shamir \& Adleman (RSA) public-key cryptography \cite{bib:RSA}, the most widely used cryptographic protocol on the internet today.
\item Simulation of quantum systems\index{Quantum algorithms!Quantum simulation} (\cite{bib:lloyd1996universal} - exponential speedup). This includes simulation of: molecular and atomic interactions in the study of quantum chemistry or nuclear physics; interactions between drug molecules and organic molecules for drug design; genetic interactions for the study of genetics and genetic medicine; nanoscale semiconductor physics for integrated circuit design; and much more.
\item Simulation of quantum field theories\index{Quantum algorithms!Quantum field theory} (\cite{bib:JLP, bib:RohdeWavelet15} - exponential speedup), a key area of fundamental physics research.
\item Topological big-data analysis\index{Quantum algorithms!Topological big-data analysis} (\cite{bib:lloyd2016quantum, USTCexperiment} - exponential speedup), with broad applications including: social media network analysis; consumer behaviour; behavioural dynamics; neuroscience; and higher-dimensional signal and image processing.
\item Solving linear systems of equations\index{Quantum algorithms!Solving linear systems} \cite{bib:harrow2009quantum, bib:BerryLinear}, with widespread applications in linear algebra and calculus.
\item Quantum machine learning\index{Quantum algorithms!Quantum machine learning} \cite{bib:lloyd2013quantum}. This includes putting an end to humanity.
\end{itemize}
...
\subsection{Amplitude damping} \index{Amplitude damping channels}
...
\subsection{Phase-space} \index{Phase-space encoding}
...
\subsubsection{Entanglement} \label{sec:ent_meas} \index{Entanglement}
...
\subsubsection{Phase-space} \index{Phase-space representation}
...
\subsection{Point-to-point (P2P)} \index{Point-to-point network topology}
...
\subsection{Complete} \index{Complete network topology}
...
\subsection{Lattice} \index{Lattice network topology}
...
\subsection{Tree} \label{sec:tree_graph} \index{Tree network topology}
...
\subsubsection{Balanced tree} \label{sec:bal_tree} \index{Balanced tree topology}
...
\subsubsection{Random tree} \index{Random tree topology}
...
\subsubsection{Minimum spanning tree} \label{sec:graph_MST} \index{Minimum spanning tree topology}
...
\subsection{Random} \index{Random topology}
...
\subsection{Minimum spanning tree} \label{sec:min_tree} \index{Minimum spanning tree}
...
\subsubsection{Coherent states} \label{sec:coherent_states} \index{Coherent states}
...
\subsubsection{Single-photons} \label{sec:single_phot_src} \index{Single-photons}
...
\subsubsection{NOON states} \label{sec:NOON} \index{NOON states}
...
\subsubsection{Cluster states} \index{Cluster states}
...
\subsubsection{Greenberger-Horne-Zeilinger states} \index{Greenberger-Horne-Zeilinger states}
...
\subsubsection{Bell states} \label{sec:bell_state_res} \index{Bell states}
...
\subsubsection{Cat states} \index{Cat states}
...
\subsubsection{Squeezed states} \label{sec:squeezed} \index{Squeezed states}
...
Essentially, the long coherence lifetimes of collective excitations owes to the fact that the excitation is effectively encoded as a W-state \cite{???}, an equal superposition of a single excitation across many atoms, of the form,
\begin{align}
...
\subsection{Universal linear optics} \label{sec:KLM_univ} \index{Universal linear optics}
...
\subsection{Weak cross-Kerr non-linearities} \index{Weak cross-Kerr non-linearities}
...
\subsection{Passive linear optics} \label{sec:passive_LO} \index{Passive linear optics}
...
As a special case, consider a single user's leverage when joining the global network. Then,
\begin{align}
...
%
% Homomorphic Encryption Of Passive Non-Interactive Optics
%

\subsection{Encryption of passive non-interactive optics} \index{Encryption of passive non-interactive optics}

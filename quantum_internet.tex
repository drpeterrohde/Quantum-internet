...
%
% Single-Photon Qubit Encodings
%

\subsection{Single-photon qubit encodings} \label{sec:single_phot_enc}
...
%
% Photon-Number Qudit Encoding
%

\subsection{Photon-number qudit encoding}
...
\subsection{Spatio-temporal qudit encoding}
...
\subsection{Time-bin qudit encoding}
...
\subsection{Continuous variable (CV) qudit encoding} \label{sec:CV_enc}
...
\subsection{Thermal state encoding}
...
\comment{Talk about Rohde, Fitzsimons, Gilchrist homomorphic boson-sampling scheme}

\comment{Talk about cluster state blind quantum computing}
...
\begin{align}
p_\mathrm{total} = {p_\mathrm{success}}^{4 n_\mathrm{logical}}.
\end{align}
(The 4 arises since there are four directions for each module to bond in). The number of logical qubits that may be prepared in the final lattice is therefore,
\begin{align}
4 n_\mathrm{logical} &= \frac{\mathrm{log}(p_\mathrm{total})}{\mathrm{log}(p_\mathrm{success})} \nonumber \\
...
n_\mathrm{logical} &= O(n_\mathrm{ancilla}).
\end{align}
...
\comment{This is bullshit. If you need as many ancillas as logical qubits, why even use ancillas?}

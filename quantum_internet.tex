...
To set the context for our upcoming treatment of quantum networks, we begin by discussing \emph{classical} networks, and defining a mathematical construction for their representation that we will subsequently generalise to the context of quantum networks. This encompasses representing networks as graphs, representing the cost of communications within the network, and how to optimise network routing to minimise costs. Importantly, we discuss the protocol stack employed by the classical internet (TCP/UDP/IP) for implementing classical networking. These notions will be essential in our treatment of quantum networks.
...
From this protocol, any given packet will eventually be successfully transmitted uncorrupted, collision-free, albeit with uncertain timing that grows with the number of competing users.
...
The computational hardness of these models for computation relate to the fact that the amplitudes in the output superpositions are proportional to matrix permanents, which are \textbf{\#P}-hard in general, yielding computationally complex sampling problems.

...
Atomic ensembles have been proposed as quantum memories, given their long coherence lifetime. Additionally, a protocol for universal cluster state quantum computation based upon atomic ensemble qubits has been described \cite{bib:RohdeAtEns10}. \comment{Add atomic ensemble refs! Get from Barrett, Rohde, Stace paper}
...
t_i &= \alpha^{n_i},
\end{align}
where \mbox{$\alpha>1$} is a constant specific to the algorithm being deployed, and the computational model (e.g cluster states vs the circuit model).

If a network of quantum computers were combined into a single, larger virtual computer, it would exhibit scaling,
\begin{align}
t_\mathrm{joint} = \alpha^{\sum_j n_j},
\end{align}
where $t_\mathrm{joint}$ is the classical-equivalent runtime of the joint quantum computation. The $i$th user is contributing the fraction of the hardware,
\begin{align}
...
&= \alpha^{\sum_j n_j} \left(\frac{\sum_j n_j}{n_i}\right).
\end{align}
...
\end{align}
Note that the economic leverage is strictly greater than unity. Thus, for any user, irrespective of the size of their quantum computer, they are better off joining the distributed network and claiming a time-share, than utilising only their own hardware.

As a special case, consider a single user's leverage when joining the global network. Then,
\begin{align}
\lambda_\mathrm{user} = \alpha^{n_\mathrm{global}} \left(1+\frac{n_\mathrm{global}}{n_\mathrm{user}}\right),
\end{align}
...
\begin{align}
\lambda_\mathrm{global} \approx \alpha^{n_\mathrm{user}},	
\end{align}
...
%
% Architectures For Cloud Quantum Computing
%

\section{Architectures for cloud quantum computing}
...
We acknowledge Charlotte Bridault, Marco Tomamichel \& Darryl Veitch for helpful discussions. P.P.R is funded by an ARC Future Fellowship (project FT160100397). We thank 1,3,7-Trimethylxanthine for aiding in the motivation for this work.
\\
\\

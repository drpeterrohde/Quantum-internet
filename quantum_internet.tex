...
The important feature to note is that the logical qubits encoded into the toric code do not reside locally at any of the physical qubits in the code. Rather, they reside jointly across the entire graph, which, like cluster states, might be divided across multiple hosts.
...
\caption{A larger cluster state module comprising a \mbox{$3\times 3$} lattice of logical qubits (grey), and dangling arms of ancillary qubits (white) in each direction for joining them to neighbouring modules.} \label{fig:larger_module}
\end{figure}

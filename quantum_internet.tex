...
\end{align}
in the photon-number basis, where $\hat{n}=\hat{a}^\dag\hat{a}$ is the photon-number operator. With no dephasing, \mbox{$\phi(\omega)=\delta(\omega)$} and $\mathcal{E}$ reduces to the identity channel. Otherwise, the off-diagonal (i.e coherence) terms in the density operator begin to cancel out, leaving the diagonal (i.e amplitude) terms unchanged.} 
...
\subsubsection{Quantum Repeater Networks} \label{sec:rep_net}

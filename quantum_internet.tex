...
\subsubsection{Passive non-interactive optics}
...
\end{align}
More specifically, we would like to maximise the von Neuman entropy of the mixture over the encoding operations,
\begin{align}
...
For,
\begin{align}
S(\hat\rho_\mathrm{encoded})=0,
\end{align}
we have no secrecy, whereas for,
\begin{align}
S(\hat\rho_\mathrm{encoded})=1,
\end{align}
we have perfect secrecy (for the given plaintext basis state).
\end{definition} 
...
Note that while we have considered linear optics in the above discussion, we could in fact expand the list of ingredients available to the computation to include anything generated by a Hamiltonian that is invariant under a polarisation rotation. This could include, for example, polarisation-independent non-linear operations.
...
This encryption technique applies to more than just linear optics, but extends to also include generalised non-linear phase-shift operations, generated by Hamiltonians that are polynomials in the photon-number operators,
\begin{align}
\hat{H} = O(\mathrm{poly}(\hat{n}_1,\dots,\hat{n}_m)).
\end{align}
This observation follows trivially from the observation that the phase-shift encoding operations (which are generated by Hamiltonians linear in the photon-number operators) commute with any polynomial in the photon-number operators,
\begin{align}
...
\comment{To do! Mathematical details! How does the secrecy of this scheme compare to the secrecy of a classical homomorphically encrypted matrix multiplication? How many bits are hidden? What mathematical techniques are used for the derivation? Add figures for trace distance with and without encryption.}

...
The entirely technically disinterested or mathematically incompetent reader may refer to just Secs.~\ref{sec:introduction} \& \ref{sec:era_quant}-\ref{sec:vision_quant} -- essentially brief non-technical essays about the motivation, applications and implications of the quantum internet.
...
\end{align}
for normalisation. Here $\mathcal{E}$ is a super-operator, denoting the action of the process on state $\hat\rho$. This is also referred to as the \emph{operator-sum representation}. This representation has the elegant interpretation as the probabilistic application of each of the Kraus operators, with probability,
\begin{align}
...
%
% Quantum Costs & Attributes
%

\section{Quantum costs \& attributes} \label{sec:quantum_meas_cost}
...
Atomic ensembles have been proposed as quantum memories, given their long coherence lifetime. Additionally, a protocol for universal cluster state quantum computation (Sec.~\ref{sec:CSQC}) based upon atomic ensemble qubits has been described \cite{bib:RohdeAtEns10}.

Essentially, the long coherence lifetimes of collective excitations owes to the fact that the excitation is effectively encoded as a W-state, an equal superposition of a single excitation across many atoms, of the form,
\begin{align}
\ket{\psi_W} = \frac{1}{\sqrt{N}}(&\ket{1,0,0,\dots}\nonumber \\
+&\ket{0,1,0,\dots}\nonumber \\
+&\ket{0,0,1,\dots}\nonumber \\
+&\dots).	
\end{align}
W-states are favourable from a decoherence perspective as tracing out a single mode/particle has minimal impact on the coherence of the residual state, which preserves most entanglement, with this robustness growing with the number of modes/particles. This is in stark contrast to GHZ states, which completely decohere under the loss of just a single mode.
...
\item Single-qubit gates are denoted as boxes containing the name of the associated unitary operation (e.g the Hadamard ($H$), Pauli bit-flip ($X$), Pauli bit-phase-flip ($Y$), and Pauli phase-flip ($Z$) gates).
...
\item Double lines represent classical feedforward, controlling a subsequent gate.
\end{itemize}
...
In Secs.~\ref{sec:dist_QC} \& \ref{sec:module} we introduced distributed and modularised quantum computation. Using this as a toy model, we will now investigate the market dynamics of uniting the quantum computational resources of multiple market participants.
...
t_i &= \beta^{n_i},
\end{align}
where \mbox{$\beta>1$} is a constant specific to the algorithm being deployed, and the computational model (e.g cluster states vs the circuit model).

If a network of quantum computers were combined into a single, larger virtual computer, it would exhibit scaling,
\begin{align}
t_\mathrm{joint} = \beta^{\sum_j n_j},
\end{align}
...
This observation leads to the natural conclusion that it is always in the best interests of quantum computer operators to contribute their resources to the distributed global network and participate in time-sharing, rather than go it alone. It follows that in the interests of economic efficiency, market forces will ensure that future quantum computers will \emph{all} be networked into a single \emph{global virtual quantum computer}, providing exponentially greater computational power to all users than what they could have afforded on their own.
...
%
% Passive Non-Interactive Optics
%

\subsection{Passive non-interactive optics}
...
Note that while we have considered linear optics in the above discussion, we could in fact expand the list of ingredients available to the computation to include anything generated by a Hamiltonian that commutes with a polarisation rotation. This could include, for example, polarisation-independent non-linear operations.
...
\cite{siHuiTan} evaluated the security of this protocol in the case where the basis states were restricted to the binary $\ket{\pm\alpha}$ states. Thus, each input mode encodes at most a single bit (zero bits for \mbox{$|\alpha|=0$}, approaching one bit for \mbox{$|\alpha|\to\infty$}). They were unable to calculate information-theoretic bounds (as is often the case), instead resorting to the alternative approach of calculating the distinguishability of codewords under the trace distance (Sec.~\ref{sec:fid_metric}). If the basis codeword states observed by Bob are indistinguishable, they are effectively decorrelated from the plaintext basis states, preventing him from guessing Alice's plaintext state, whereas if they are distinguishable, he can.
...
\begin{align}
||\hat\rho_{\vec{x}}-\hat\rho_{\vec{0}}||_\mathrm{tr} = \sqrt{1-e^{-4\mathrm{wt}(\vec{x})|\alpha|^2}},
\end{align}
where $\mathrm{wt}(\vec{x})$ is the Hamming weight (number of 1's in the bit-string) of $\vec{x}$. On the other hand, for the encoded states, we have,
\begin{align}
&||\mathcal{E}(\hat\rho_{\vec{x}})-\mathcal{E}(\hat\rho_{\vec{0}})||_\mathrm{tr} \nonumber \\
&= \sum_{k=0}^{d-1} e^{-m|\alpha|^2}\frac{(m|\alpha|^2)^k}{k!}\sqrt{1-\left(\frac{m-2\mathrm{wt}(\vec{x})}{m}\right)^{2k}}
\end{align}
\comment{What's the limit as $d\to\infty$?} where $\mathcal{E}$ denotes the mixture over encoding operations observed by Bob, as per Eq.~\ref{eq:mix_over_enc_ops}, and there are $d$ distinct keys. These relationships are illustrated in Fig.~\ref{fig:homo_coh_st_tr}\footnote{Note that the distance between any arbitrary pair of codewords may be obtained by replacing $\mathrm{wt}(\cdot)$ with their Hamming distance (in the simple case where one of the codewords is the $\vec{0}$ bit-string, the Hamming weight and Hamming distance are equivalent). \comment{Is this right?}}. Clearly, the encoded basis states exhibit greater indistinguishability than the unencoded ones, demonstrating that information is hidden from Bob. Unfortunately, the trace distance does not have the elegant interpretation of `number of bits hidden' that the mutual information does. Nonetheless, this demonstrates partial hiding of information.

\begin{figure}[!htb]
\includegraphics[width=\columnwidth]{coherent_state_homo_unenc} \\
\includegraphics[width=\columnwidth]{coherent_state_homo_enc}
\caption{Trace distance between unencoded (top) and encoded (bottom) basis states for coherent state computation using phase-key encoding, with \mbox{$d=50$} keys. Each of the \mbox{$m=10$} modes is inputted with one of two basis coherent states, $\ket{\pm\alpha}$. $\mathrm{wt}(\vec{x})$ denotes the Hamming weight of bit-string $\vec{x}$. Two states are indistinguishable if their trace distance is 0, and distinguishable if their trace distance is 1. Lower trace distance between encoded states implies a lower chance of Bob guessing Alice's plaintext input state.} \label{fig:homo_coh_st_tr}
\end{figure}
...
\begin{align}
\hat{D}_i(\alpha_i) \to \bigotimes_{j=1}^m \hat{D}_j(\beta_j),
\end{align}
...
We acknowledge Charlotte Bridault, Marco Tomamichel \& Darryl Veitch for helpful discussions. We thank the authors of QCircuit ({\tt \href{http://physics.unm.edu/CQuIC/Qcircuit/}{http://physics.unm.edu/CQuIC/Qcircuit/}}) for providing this very helpful package. P.P.R is funded by an ARC Future Fellowship (project FT160100397). We thank 1,3,7-Trimethylxanthine for aiding in the motivation for this work.
\\
\\

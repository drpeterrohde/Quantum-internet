...
I(X:Z)&\leq I(Y:Z),
\end{align}
with equality only if one of the channels is the identity channel, i.e one of the links in the chain perfectly preserves information content.
...
Atomic ensembles have been proposed as quantum memories, given their long coherence lifetime. Additionally, a protocol for universal cluster state quantum computation based upon atomic ensemble qubits has been described \cite{???}. \comment{Add atomic ensemble refs! Get from Barrett, Rohde, Stace paper}
...
We can express these requirements very generally and elegantly in terms of a commutation relation between the encoding ($\hat{E}$), decoding ($\hat{D}$), and computational ($\hat{U}$) operations,
...
\end{align}
where $\hat{E}_i(k_i)$ and $\hat{D}_i(\tilde k_i)$ are the encoding and decoding operations for the $i$th mode, and the key $k$ is partitioned into distinct blocks $\{k_i\}$, one for each mode, such that \mbox{$k=\{k_1,\dots,k_m\}$}. $\tilde k$ is a potentially transformed version of $k$, to accommodate that the encryption and decryption keys may be distinct.
...
\caption{General structure for the relationship between the encoding ($\hat{E}$), decoding ($\hat{D}$), and computational ($\hat{U}$) operations in a passive, non-interactive, optical quantum computation, where Alice is restricted to non-entangling, single-mode operations.} \label{fig:gen_pass_hom}
\end{figure}
...
\end{align}
This immediately significantly expands the class of operations available for the computation. In particular, while coherent states remain separable under linear optics evolution, the introduction of non-linear phase-shift operations enables entanglement, and presumably a more powerful class of computations than simple matrix multiplication.
